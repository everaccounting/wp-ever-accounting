 ...
 */
const packages = [ '@eaccounting/components' ];

const NAMESPACE = '@eaccounting/';
...
const requestToExternal = ( request ) => {
	if ( packages.includes( request ) ) {
		const handle = request.substring( NAMESPACE.length );

		return [ 'eaccounting', camelCaseDash( handle ) ];
	}
};
...
class DependencyExtractionWebpackPlugin extends WPDependencyExtractionWebpackPlugin {
	externalizeWpDeps( _context, request, callback ) {
		...
		if (
			...
			externalRequest = requestToExternal( request );
		}
