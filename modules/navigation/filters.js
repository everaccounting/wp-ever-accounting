 ...
 */
import { find, get, omit, uniqWith, isEqual } from 'lodash';
 ...
 */
export function getActiveFiltersFromQuery( query, filters ) {
	return Object.keys( filters ).reduce( ( activeFilters, configKey ) => {
		const filter = filters[ configKey ];
		if ( filter.rules ) {
		...
		return uniqWith( activeFilters, isEqual );
	}, [] );
}
 ...
 */
export function getDefaultOptionValue( filter, options ) {
	const { defaultOption } = filter.input;
	if ( filter.input.defaultOption ) {
		const option = find( options, { value: defaultOption } );
		if ( ! option ) {
			/* eslint-disable no-console */
			console.warn(
				`invalid defaultOption ${ defaultOption } supplied to ${ filter.labels.add }`
			);
			/* eslint-enable */
			return undefined;
		}
 ...
 */
export function getQueryFromActiveFilters(
	activeFilters = [],
	query,
	filters
) {
	const previousFilters = getActiveFiltersFromQuery( query, filters );
	...
	const nextData = activeFilters.reduce( ( data, filter ) => {
		...
		if ( filter.value ) {
			...
			if (
				filters[ filter.key ] &&
				filters[ filter.key ].allowMultiple
			) {
