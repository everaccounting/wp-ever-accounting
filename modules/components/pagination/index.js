	...
	return (
		<span className="pagination-links">
			<Nav
				...
				enabled={current > 0 && current !== 1 && !isDisabled}
				onClick={() => setPage(1)}
			/>
			&nbsp;
			<Nav
			...
			<Nav
				...
				button="›"
				className="next-page"
				enabled={current < max && !isDisabled}
				onClick={() => setPage(current + 1)}
			/>
			&nbsp;
			<Nav
				...
				button="»"
				className="last-page"
				enabled={current < max - 1 && !isDisabled}
				onClick={() => setPage(max)}
			/>
		</span>
	);
...
const Pagination = ({ perPage, page = 1, total, onPageChange, isDisabled }) => {
