...
const queriedTotalCacheByState = new WeakMap();
/**
...
function getQueriedItemsUncached( state, query ) {
	const { stableKey, page, per_page, include, fields } = getQueryParts(
		query
	);
	let itemIds;
	...
	const endOffset =
		per_page === -1
			? itemIds.length
			: Math.min( startOffset + per_page, itemIds.length );
...
export const getQueriedTotal = createSelector( ( state, query = {} ) => {
	...
	if ( queriedTotalCache ) {
		const queriedTotal = queriedTotalCache.get( query );
		if ( queriedTotal !== undefined ) {
			return queriedTotal;
		}
	} else {
 ...
 */
export const getQueriedError = createSelector( ( state, query = {} ) => {
	const { stableKey } = getQueryParts( query );
	return get( state, [ stableKey ], false );
} );
