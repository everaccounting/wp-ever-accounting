 ...
 * @param name
 * @param schema
 * @returns {{schema, name, type: string}}
 */
export function receiveEntitySchema(name, schema) {
	return {
...
export function* editEntity(name, recordId, edits, options = {}) {
	...
	const record = yield select(STORE_KEY, 'getRawEntity', name, recordId);
		...
		for (const [key, value] of Object.entries(record)) {
			if (typeof value === 'function') {
				const evaluatedValue = value(
					yield select(STORE_KEY, 'getEditedEntity', name, recordId)
				);
				yield editEntity(
					name,
					recordId,
