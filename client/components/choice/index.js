/**
 * External dependencies
 */


/**
 * Internal dependencies
 */
import Dropdown from '../dropdown';
import DotMenuIcon from '../icons/dot-menu';
import './style.scss';

/**
 * Display a dropdown menu behind a toggle button. The menu items contain the logic to detect clicks, if needed.
 *
 * @param {object} props - Component props
 * @param {object[]} props.menu - Array of menu items (React components)
 * @param {boolean} [props.disabled=false] - Disable the menu
 * @param {('left'|'right')} [props.align] - Which side the menu appears on
 */
function Choice( { menu, align = 'right', disabled = false } ) {
	return (
		<Dropdown
			align={ align }
			hasArrow
			renderToggle={ ( isOpen, toggle ) => (
				<button type="button" className="ea-dropdownmenu" onClick={ toggle } disabled={ disabled }>
					<DotMenuIcon isOpen={ isOpen } />
				</button>
			) }
			renderContent={ ( toggle ) => (
				<ul className="ea-dropdownmenu__menu" onClick={ toggle }>
					{ menu.map( ( item, key ) => (
						<li key={ key }>{ item }</li>
					) ) }
				</ul>
			) }
		/>
	);
}

export default Choice;
