			...
			{Object.keys(filters).map((key, idx) => {
				...
				return (
					<FilterComponent
						...
						filter={{...filter, input: {...filter.input, value: get(applied, [key], defaultVal)}}}
						onFilterChange={partial(setFilter, key)}
					/>
				)
			})}

			{!isEmpty(flatten(applied)) && <Button onClick={reset} isSecondary>{__('Clear all filters', 'wp-ever-accounting')}</Button>}
		</div>
	)
}
