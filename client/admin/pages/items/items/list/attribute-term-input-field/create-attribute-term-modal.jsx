...
export const CreateAttributeTermModal = ( {
	...
	const onAdd = async ( attribute ) => {
		...
		try {
			...
			createNotice( 'error', __( 'Failed to create attribute term.', 'woocommerce' ) );
			setIsCreating( false );
			onCancel();
		}
	};
	function validateForm( values ) {
		...
		if ( ! values.name?.length ) {
			errors.name = __( 'The attribute term name is required.', 'woocommerce' );
		}
		return errors;
	}
	return (
		...
		>
			<Form
				...
				onSubmit={ onAdd }
			>
				{ ( { getInputProps, handleSubmit, isValidForm, setValue, values } ) => {
