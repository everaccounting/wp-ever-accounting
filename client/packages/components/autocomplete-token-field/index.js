			...
			request
				.then((suggestions) => {
					...
					suggestions.forEach((suggestion) => {
						const duplicatedSuggestionIndex =
							currentSuggestions.indexOf(suggestion.label);
	...
	render() {
		...
		return (
			<div className="autocomplete-tokenfield">
				...
				{loading && <Spinner />}
				{help && (
					<p className="autocomplete-tokenfield__help">{help}</p>
				)}
			</div>
		);
	}
