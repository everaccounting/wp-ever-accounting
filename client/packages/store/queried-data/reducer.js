 ...
 */
function removeItemsById(entities, ids) {
...
function items(state = {}, action) {
	switch (action.type) {
		case 'RECEIVE_ITEMS':
			...
			const key = action.key || DEFAULT_PRIMARY_KEY;
			return {
				...state,
				[context]: {
					...state[context],
		...
		case 'REMOVE_ITEMS':
			return Object.fromEntries(
				Object.entries(state).map(([itemId, contextState]) => [
					itemId,
					removeItemsById(contextState, action.itemIds),
				])
			);
	}
	return state;
...
export function itemIsComplete(state = {}, action) {
	switch (action.type) {
		case 'RECEIVE_ITEMS': {
			const context = getContextFromAction(action);
			const { query, key = DEFAULT_PRIMARY_KEY } = action;
...
const receiveQueries = compose([
	...
	const { type, page, perPage, key = DEFAULT_PRIMARY_KEY } = action;
