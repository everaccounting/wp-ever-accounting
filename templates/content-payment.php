 ...
 * @version 1.0.0
 *
 * @var \EverAccounting\Models\Payment $payment
 */
...
<div class="eac-panel is-large eac-document is--payment mt-0">
	<div class="eac-document__header">
		...
		<div class="eac-document__title">
			<div
				class="eac-document__title-text is-medium"><?php esc_html_e( 'Payment Receipt', 'wp-ever-accounting' ); ?></div>
			<div class="eac-document__title-meta">#<?php echo esc_html( $payment->get_number() ); ?></div>
		</div>
	</div>
		...
		<div class="eac-document__section document-items">
			...
			<table class="eac-document__items">
				...
				<tr>
					...
					<td>
						<?php echo esc_html( $payment->get_date() ); ?>
					</td>
				</tr>
				<tr>
					<th scope="col">
					...
					<td>
						<?php if ( $payment->get_payment_method() && array_key_exists( $payment->get_payment_method(), eac_get_payment_methods() ) ) : ?>
							<?php echo esc_html( eac_get_payment_methods()[ $payment->get_payment_method() ] ); ?>
						<?php else : ?>
				...
				<tr>
					...
					<td>
						<?php if ( $payment->get_reference() ) : ?>
							<?php echo esc_html( $payment->get_reference() ); ?>
						<?php else : ?>
							<?php echo esc_html( 'N/A' ); ?>
						<?php endif; ?>
					</td>
				...
				<tr>
					...
					<td>
						<?php echo esc_html( $payment->get_formatted_amount() ); ?>
					</td>
				</tr>
				</tbody>
			</table>
		</div>
		<?php if ( $payment->get_note() ) : ?>
			<div class="eac-document__section document-notes">
				...
				<?php echo wp_kses_post( $payment->get_note() ); ?>
			</div>
		<?php endif; ?>
	</div>
</div>
