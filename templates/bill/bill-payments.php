<?php
/**
 * Displays bill payments.
 *
 * This template can be overridden by copying it to yourtheme/eaccounting/bill/invoice-payments.php.
 *
 * @var $bill
 * @version 1.1.0
 */
	...
	<?php else : ?>
		<table class="ea-card__body ea-invoice-payment widefat" style="border: 0;">
			...
			<?php foreach ( $payments as $payment ) : ?>
				<tr>
					<td>
						<a href="<?php echo add_query_arg(array('page' => 'ea-expenses', 'tab' => 'bills', 'revenue_id' => $payment->get_id(), 'action'=> 'edit'), admin_url('admin.php'));//phpcs:ignore?>">
					...
					<td>
						<abbr title="<?php echo esc_attr( eaccounting_price( $payment->get_amount(), $payment->get_currency_code() ) ); ?>">
							<?php echo esc_html( eaccounting_price( eaccounting_price_convert( $payment->get_amount(), $payment->get_currency_code(), $payment->get_currency_rate(), $bill->get_currency_code(), $bill->get_currency_rate() ), $bill->get_currency_code() ) ); ?>
						</abbr>
					</td>
				</tr>
			<?php endforeach; ?>
