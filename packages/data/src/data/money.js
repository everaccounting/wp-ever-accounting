...
export default class Money {
	...
	/**
	 * Returns the value of this Money as its subunit.
	 * @return {number} If the subunit is 100 and the value is .45,
	 * this returns 450
	 */
	tosubunit() {
		return this.amount.toNumber() * this.currency.subunit;
	}
	...
	allocate( ratios ) {
		...
		let remainder = new Decimal( self.tosubunit() );
		...
		convertedRatios.forEach( ( ratio ) => {
			const share = new Decimal(
				Math.floor(
					self.tosubunit() * ratio.toNumber() / total.toNumber()
				)
			);
			results.push(
				new Money(
					share.dividedBy( this.currency.subunit ),
					this.currency
				)
			);
		...
		for ( let i = 0; remainder.greaterThan( 0 ); i++ ) {
			results[ i ] = new Money(
				( new Decimal( results[ i ].tosubunit() ) )
					.plus( 1 )
					.dividedBy( this.currency.subunit ),
				this.currency
			);
	 ...
	 * rounded to `precision` using `rounding` mode.
	 *
	 * If the value of this instance in normal notation has fewer than
	 * precision fraction digits, the return value will be appended with
	 * zeros accordingly.
	 *
	 ...
	 * rounding mode.
	 */
	toFixed( precision, rounding = Money.ROUND_HALF_UP ) {
		precision = precision || this.currency.precision;
		return this.amount.toFixed( precision, rounding );
	}
