 ...
 * @return {Object} Action object.
 */
export function receiveRecords( name, records, query = {}, key = 'id', invalidateCache = false, edits ) {
	return {
	...
	async ( { select, dispatch } ) => {
		...
		for ( const [ key, value ] of Object.entries( record ) ) {
			if ( typeof value === 'function' ) {
				...
				await dispatch( editRecord( name, recordId, { [ key ]: evaluatedValue } ) );
		...
		try {
			...
			await dispatch( removeRecords( name, response, {}, key, invalidateCache ) );
		} catch ( _error ) {
			error = _error;
		}
...
export const editRecord =
	( name, recordId, edits ) =>
	async ( { select } ) => {
		const entity = select.getEntity( name );
