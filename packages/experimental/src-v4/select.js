...
function Select(props) {
	...
	const renderClearIcon = () => {
		...
		const innerProps = {
			'aria-hidden': true,
			className: 'eac-select-control__clear-indicator',
			onMouseDown: (event) => {
				event.stopPropagation();
				event.preventDefault();
				onInputChange('');
	...
	const renderLoadingIcon = () => {
		...
		const innerProps = {
			'aria-hidden': true,
			className: 'eac-select-control__loading-indicator',
		};
	...
	const renderMenu = () => {
		...
		if (options && options.length > 0) {
			...
			if (message === null) return null;
			menuUI = (
				<div className="eac-select-control__create-option" onClick={handleCreateOption}>
					{message}
				</div>
			);
		} else {
