...
export class Currency {
	...
	constructor(currencyConfig) {
		...
		this.signB4 = isUndefined(currencyConfig.signB4) ? this.signB4 : currencyConfig.signB4;
		this.decimalPlaces = isUndefined(currencyConfig.decimalPlaces) ? this.decimalPlaces : currencyConfig.decimalPlaces;
	...
	static validateCurrencyConfig = config => {
		if (isEmpty(config)) {
			throw new Exception('The configuration object provided to Currency must not' + ' be empty');
		}
		if (!config.code || !isString(config.code)) {
			throw new TypeError(
				'The configuration object provided to Currency must have ' + 'a "code" property that is a string.'
			);
		}

		if (!config.sign || !isString(config.sign)) {
			throw new TypeError(
				'The configuration object provided to Currency must have a ' + '"sign" property that is a string.'
			);
		}
		...
		if (config.thousandsSeparator && !isString(config.thousandsSeparator)) {
			throw new TypeError(
				'The thousandsSeparator property on the configuration object ' + 'must be a string primitive.'
			);
		}
...
export const SiteCurrency = (config = {}) => {
	...
	try {
		...
		currency = {};
		warning(false, 'The Site Currency object could not be created because ' + 'of this error: ' + e.message);
	}
	return currency;
};
