...
function CustomerControl(props) {
	const fetchAPI = (params, callback) => {
		console.log(params);
		apiFetch({path: addQueryArgs('/ea/v1/customers', {...params})}).then(res => {
			callback(res);
		});
	}
	...
	return (
		<AsyncSelect
			...
			noOptionsMessage={(input) => {
				return input.inputValue ? __('No customers', 'wp-ever-accounting') : __('Type to search', 'wp-ever-accounting');
			}}
			{...props}
		/>
	)
