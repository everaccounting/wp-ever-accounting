...
const Select = React.forwardRef((props, ref) => {
	...
	const displayValues = React.useMemo(() => {
		...
		return mergedValues.map(item => ({
			...item,
			label:
				(typeof labelRender === "function" ? labelRender(item) : item.label) ??
				item.value
		}))
	...
	const filledSearchOptions = React.useMemo(() => {
		if (
			...
			filteredOptions.some(
				item => item[optionFilterProp || "value"] === mergedSearchValue
			)
		) {
	...
	const onActiveValue = React.useCallback(
