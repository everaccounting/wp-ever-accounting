...
export const Menu = ( {
	...
	useLayoutEffect( () => {
		const comboboxWrapper = selectControlMenuRef.current?.closest(
			'.eac-select-control__combo-box-wrapper'
		);
	...
	return (
		<div ref={ selectControlMenuRef } className="eac-select-control__menu">
			<div>
				<Popover
					...
					__unstableSlotName="eac-select-control-menu"
					focusOnMount={ false }
					className={ classnames( 'eac-select-control__popover-menu', {
						'is-open': isOpen,
				...
				>
					<ul
						...
						className={ classnames(
							'eac-select-control__popover-menu-container',
							className
						) }
						onMouseUp={ ( e ) =>
...
export const MenuSlot = () =>
	createPortal(
		<div aria-live="off">
			...
			<Popover.Slot name="eac-select-control-menu" />
		</div>,
		document.body
	);
