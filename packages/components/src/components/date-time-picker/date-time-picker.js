...
import { useCallback, useState, useEffect, useMemo, useRef, forwardRef } from '@wordpress/element';
...
export const DateTimePicker = forwardRef( function ForwardedDateTimePickerControl(
	{
		currentDate,
	...
	const id = useInstanceId( DateTimePicker, 'inspector-date-time-picker-control', props.id );
	...
	function parseAsISODateTime( dateString, assumeLocalTime = false ) {
		return assumeLocalTime ? moment( dateString, moment.ISO_8601, true ).utc() : moment.utc( dateString, moment.ISO_8601, true );
	}
	...
	const [ inputString, setInputString ] = useState( currentDateTime.isValid() ? formatDateTimeForDisplay( maybeForceTime( currentDateTime ) ) : '' );
	...
	const setInputStringAndMaybeCallOnChange = useCallback(
		( newInputString, isUserTypedInput ) => {
	...
	const debouncedSetInputStringAndMaybeCallOnChange = useDebounce( setInputStringAndMaybeCallOnChange, onChangeDebounceWait );
	...
	return (
		<Dropdown
			className={ classNames( 'eac-date-time-picker-control', className ) }
			focusOnMount={ false }
				...
				<BaseControl id={ id } label={ label } help={ help }>
					<InputControl
						...
						suffix={
							<Icon
								icon={ calendar }
								className="calendar-icon eac-date-time-picker-control__input-control__suffix"
						...
						describedBy={ sprintf(
							...
							__( 'Date input describing a selected date in format %s', 'eac' ),
							dateTimeFormat
						) }
						onFocus={ () => {
							if ( isOpen ) {
			...
			popoverProps={ {
				...
				className: 'eac-date-time-picker-control__popover',
