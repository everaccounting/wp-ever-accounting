 ...
 */
import { useControlledValue, useControlledState } from '../utils';
...
function Table( props ) {
	...
	const { isExpanded, isAllExpanded, onExpandItem, onExpandAll } = useExpandable( data );
	...
	// const setRowExpanded = ( row ) => {
	// 	// toggle the row.
	// 	const index = findIndex( expandedRows, ( expandedRow ) => isEqual( expandedRow, row ) );
	// 	if ( index > -1 ) {
	// 		setExpandedRows( expandedRows.filter( ( expandedRow ) => ! isEqual( expandedRow, row ) ) );
	// 	} else {
	...
	const searchNode = useMemo( () => {
		...
		return (
			<SearchControl
				...
				value={ query.search }
				onChange={ ( keyword ) => setQuery( { ...query, search: keyword, page: 1 } ) }
	...
	const alertSection = useMemo( () => {
		...
		return (
			<div className="eac-table__section eac-table__section--alert">
				<div className="eac-table__alert">
					{ sprintf( __( '%d items selected', 'wp-ever-accounting' ), selectedRows.length ) }
				</div>
			</div>
		);
	}, [ selectedRows ] );
