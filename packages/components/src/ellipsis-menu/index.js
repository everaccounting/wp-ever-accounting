...
const EllipsisMenu = ( { label, renderContent, className, onToggle } ) => {
	...
	const renderMenu = ( renderContentArgs ) => <NavigableMenu className="eac-ellipsis-menu__content">{ renderContent( renderContentArgs ) }</NavigableMenu>;

	return (
		<div className={ classnames( className, 'eac-ellipsis-menu' ) }>
			<Dropdown contentClassName="eac-ellipsis-menu__popover" position="bottom left" renderToggle={ renderEllipsis } renderContent={ renderMenu } />
		</div>
	);
};
