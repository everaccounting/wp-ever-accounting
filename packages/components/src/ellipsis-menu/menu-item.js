...
const MenuItem = ({ checked, children, isCheckbox = false, isClickable = false, onInvoke = () => {} }) => {
		...
		if (eventTarget.isSameNode(event.currentTarget)) {
			...
			if (event.keyCode === DOWN) {
				...
				const nextElementToFocus =
					eventTarget.nextSibling || eventTarget.parentNode?.querySelector('.eac-ellipsis-menu__item');
	...
	if (isCheckbox) {
		return (
			<div
				...
				onKeyDown={onKeyDown}
				onClick={onClick}
				className="eac-ellipsis-menu__item"
			>
	...
	return (
		<div role="menuitem" tabIndex={0} onKeyDown={onKeyDown} onClick={onClick} className="eac-ellipsis-menu__item">
			{children}
		</div>
	);
};
