...
function TableRender( props ) {
	 ...
	 * @return
	 */
	const editableDataSource = ( dataSource ) => {
	...
	const tableAreaDom =
		...
		propsCardProps === false || notNeedCardDom === true || !! props.name ? (
			...
			<ProCard ghost={ props.ghost } bordered={ isBordered( 'table', cardBordered ) } bodyStyle={ cardBodyStyle } { ...propsCardProps }>
				{ tableContentDom }
			</ProCard>
		);
	...
	const proTableDom = (
		...
		>
			...
			{ type !== 'form' && props.tableExtraRender && (
				<div className={ classNames( className, `${ defaultClassName }-extra` ) }>{ props.tableExtraRender( props, action.dataSource || [] ) }</div>
			) }
...
const ProTable = ( props ) => {
	...
	/** 单选多选的相关逻辑 */
	const [ selectedRowKeys, setSelectedRowKeys ] = useMountMergeState( propsRowSelection ? propsRowSelection?.defaultSelectedRowKeys || [] : undefined, {
	...
	const [ proSort, setProSort ] = useMountMergeState( {} );
	...
	const searchNode = useMemo( () => {
		...
		return node;
	}, [ beforeSearchSubmit, formRef, ghost, loading, manualRequest, onFormSearchSubmit, pagination, props, propsColumns, search, searchFormRender, type ] );

	const selectedRows = useMemo( () => selectedRowKeys?.map( ( key ) => preserveRecordsRef.current?.get( key ) ), [ selectedRowKeys ] );
