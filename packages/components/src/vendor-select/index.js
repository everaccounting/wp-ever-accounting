	...
	render() {
		return (
			<Fragment>
				...
				<Modal title={__('New Vendor')} onClose={this.props.onClose}>
					<Form
...
export default class VendorSelect extends Component {
	static propTypes = {
	...
	fetchAPI(params, callback) {
		apiFetch({path: addQueryArgs('/ea/v1/contacts', {...params, type: 'vendor'})}).then(res => {
			callback(res);
		});
	}
	...
	render() {
		return (
			<Fragment>
				{this.state.isAdding && <CreateVendorModal
