...
export const Date = forwardRef( function ForwardedDate(
	{
		currentDate,
	...
	const id = useInstanceId( Date, 'inspector-date-time-picker-control', props.id );
	...
	const setInputStringAndMaybeCallOnChange = useCallback(
		( newInputString, isUserTypedInput ) => {
			...
			if ( typeof onChangeRef.current === 'function' && ! isDateTimeSame ) {
				onChangeRef.current(
					newDateTime.isValid() ? formatDateTimeAsISO( newDateTime ) : lastTypedValue,
					newDateTime.isValid()
				);
			}
		},
			...
			renderToggle={ ( { isOpen, onClose, onToggle } ) => (
				<BaseControl id={ id } label={ label } help={ help }>
					<InputControl
						...
						onChange={ ( newValue ) =>
							debouncedSetInputStringAndMaybeCallOnChange( newValue, true )
						}
						onBlur={ ( event ) => {
				...
				return (
					<Picker
						...
						onChange={ ( newDateTimeISOString ) =>
							setInputStringAndMaybeCallOnChange( newDateTimeISOString, false )
						}
