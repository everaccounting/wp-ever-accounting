...
function eaccounting_reports_income_summary_tab() {
	$year        = filter_input( INPUT_GET, 'year', FILTER_SANITIZE_NUMBER_INT, array( 'options' => array( 'defaults' => wp_date( 'Y' ) ) ) );
	...
	<div class="ea-card is-compact">
		<form action="" class="ea-report-filter">
			...
			submit_button( esc_html__( 'Filter', 'wp-ever-accounting' ), 'action', false, false );
			?>
		</form>
	</div>
	<div class="ea-card">
		...
		$date       = new \EverAccounting\Legacy\DateTime( $start );
		// Dates.
		for ( $j = 1; $j <= 12; $j ++ ) {
		...
		$chart = new \EverAccounting\Legacy\Chart();
