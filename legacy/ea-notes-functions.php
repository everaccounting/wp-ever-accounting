 ...
 *
 * @return EverAccounting\Legacy\Models\Note|null
 * @since 1.1.0
 */
function eaccounting_get_note( $item ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Note( $item );
 ...
 * @since 1.1.0
 *
 * @return \EverAccounting\Legacy\Models\Note|false|int|WP_Error
 */
function eaccounting_insert_note( $args, $wp_error = true ) {
	...
	try {
		...
		// Retrieve the item.
		$item = new \EverAccounting\Legacy\Models\Note( $args['id'] );
...
function eaccounting_delete_note( $note_id ) {
	try {
		$item = new EverAccounting\Legacy\Models\Note( $note_id );
...
function eaccounting_get_notes( $args = array() ) {
	...
	$table        = \EverAccounting\Legacy\Repositories\Notes::TABLE;
	$columns      = \EverAccounting\Legacy\Repositories\Notes::get_columns();
