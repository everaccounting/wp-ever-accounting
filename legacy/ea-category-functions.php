 ...
 *
 * @return null|EverAccounting\Legacy\Models\Category
 * @since 1.1.0
 */
function eaccounting_get_category( $category ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Category( $category );
 ...
 * @param string $type Category type.
 *
 * @return \EverAccounting\Legacy\Models\Category|null
 * @since 1.1.0
 */
 ...
 *
 * @return int|\WP_Error|\EverAccounting\Legacy\Models\Category|bool The value 0 or WP_Error on failure. The Category object on success.
 * @since 1.1.0
 */
function eaccounting_insert_category( $data = array(), $wp_error = true ) {
	...
	try {
		...
		$item = new \EverAccounting\Legacy\Models\Category( $data['id'] );
...
function eaccounting_delete_category( $category_id ) {
	try {
		$category = new EverAccounting\Legacy\Models\Category( $category_id );
...
function eaccounting_get_categories( $args = array() ) {
	...
	$table        = \EverAccounting\Legacy\Repositories\Categories::TABLE;
	$columns      = \EverAccounting\Legacy\Repositories\Categories::get_columns();
