<?php
/**
 * EAccounting Document Functions.
 *
 ...
 *
 * @return EAccounting\Models\Invoice|null
 */
function eaccounting_get_invoice( $invoice ) {
	...
	try {
		$result = new EAccounting\Models\Invoice( $invoice );
...
function eaccounting_insert_invoice( $args, $wp_error = true ) {
	...
	try {
		...
		// Retrieve the item.
		$item = new \EAccounting\Models\Invoice( $args['id'] );
...
function eaccounting_delete_invoice( $invoice_id ) {
	try {
		$invoice = new EAccounting\Models\Invoice( $invoice_id );
 ...
 *
 * @return EAccounting\Models\Bill|null
 */
function eaccounting_get_bill( $bill ) {
	...
	try {
		$result = new EAccounting\Models\Bill( $bill );
...
function eaccounting_insert_bill( $args, $wp_error = true ) {
	...
	try {
		...
		// Retrieve the item.
		$item = new \EAccounting\Models\Bill( $args['id'] );
...
function eaccounting_delete_bill( $bill_id ) {
	try {
		$bill = new EAccounting\Models\Bill( $bill_id );
...
function eaccounting_get_documents( $args = array() ) {
	...
	$table        = \EAccounting\Repositories\Documents::TABLE;
	$columns      = \EAccounting\Repositories\Documents::get_columns();
			...
			function ( $item ) {
				switch ( $item->type ) {
					case 'invoice':
						...
						break;
					case 'bill':
						$document = new \EAccounting\Models\Bill();
						$document->set_props( $item );
						$document->set_object_read( true );
						break;
					default:
