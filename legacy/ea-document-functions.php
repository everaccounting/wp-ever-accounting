 ...
 *
 * @return EverAccounting\Legacy\Models\Invoice|null
 */
function eaccounting_get_invoice( $invoice ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Invoice( $invoice );
...
function eaccounting_insert_invoice( $args, $wp_error = true ) {
	...
	try {
		...
		// Retrieve the item.
		$item = new \EverAccounting\Legacy\Models\Invoice( $args['id'] );
...
function eaccounting_delete_invoice( $invoice_id ) {
	try {
		$invoice = new EverAccounting\Legacy\Models\Invoice( $invoice_id );
 ...
 *
 * @return EverAccounting\Legacy\Models\Bill|null
 */
function eaccounting_get_bill( $bill ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Bill( $bill );
...
function eaccounting_insert_bill( $args, $wp_error = true ) {
	...
	try {
		...
		// Retrieve the item.
		$item = new \EverAccounting\Legacy\Models\Bill( $args['id'] );
...
function eaccounting_delete_bill( $bill_id ) {
	try {
		$bill = new EverAccounting\Legacy\Models\Bill( $bill_id );
...
function eaccounting_get_documents( $args = array() ) {
	...
	$table        = \EverAccounting\Legacy\Repositories\Documents::TABLE;
	$columns      = \EverAccounting\Legacy\Repositories\Documents::get_columns();
			...
			function ( $item ) {
				switch ( $item->type ) {
					case 'invoice':
						...
						break;
					case 'bill':
						$document = new \EverAccounting\Legacy\Models\Bill();
						$document->set_props( $item );
						$document->set_object_read( true );
						break;
					default:
