...
use EverAccounting\Legacy\Models\Currency;
 ...
 *
 * @return EverAccounting\Legacy\Models\Currency|null
 * @since 1.1.0
 */
function eaccounting_get_currency( $currency ) {
	...
	try {
		return new EverAccounting\Legacy\Models\Currency( $currency );
	} catch ( \Exception $e ) {
		return null;
	}
}
 ...
 *
 * @return EverAccounting\Legacy\Models\Currency|\WP_Error|bool
 * @since 1.1.0
 */
function eaccounting_insert_currency( $args, $wp_error = true ) {
	...
	try {
		...
		$item = new \EverAccounting\Legacy\Models\Currency( $args );
...
function eaccounting_delete_currency( $currency_code ) {
	try {
		$currency = new EverAccounting\Legacy\Models\Currency( $currency_code );
...
function eaccounting_get_currencies( $args = array() ) {
	...
	$option     = \EverAccounting\Legacy\Repositories\Currencies::OPTION;
	$columns    = \EverAccounting\Legacy\Repositories\Currencies::get_columns();
