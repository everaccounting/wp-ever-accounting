...
// Functions.
require_once EACCOUNTING_LEGACY_PATH . '/ea-account-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-misc-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-formatting-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-form-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-file-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-currency-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-transaction-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-category-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-contact-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-notes-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-deprecated-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-item-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-tax-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-document-functions.php';
require_once EACCOUNTING_LEGACY_PATH . '/ea-template-functions.php';
...
function eaccounting_get_financial_start( $year = null, $format = 'Y-m-d' ) {
	...
	$financial_year = new \EverAccounting\Legacy\DateTime();
...
function eaccounting_get_financial_end( $year = null, $format = 'Y-m-d' ) {
	$dt = new \EverAccounting\Legacy\DateTime( eaccounting_get_financial_start( $year, 'Y-m-d' ) );
 ...
 *
 * @return \EverAccounting\Legacy\Money|WP_Error
 */
function eaccounting_money( $amount, $code = 'USD', $convert = false ) {
	try {
		return new \EverAccounting\Legacy\Money( $amount, $code, $convert );
	} catch ( Exception $e ) {
 ...
 *
 * @since 1.0.2
 * @return \EverAccounting\Legacy\Logger
 */
...
function eaccounting_cleanup_logs() {
	$logger = new \EverAccounting\Legacy\Logger();
 ...
 *
 * @return \EverAccounting\Legacy\Collection
 */
function eaccounting_collect( $items ) {
	return new \EverAccounting\Legacy\Collection( $items );
}
...
function eaccounting_get_data( $key ) {
	...
	$data = apply_filters( "eaccounting_get_data_$key", include EACCOUNTING_LEGACY_PATH . "/data/$key.php" );
...
function eaccounting_init_license( $file, $item_name ) {
	if ( is_admin() && class_exists( '\EverAccounting\Legacy\License' ) ) {
		$license = new \EverAccounting\Legacy\License( $file, $item_name );
	}
}
