...
use EverAccounting\Legacy\Models\Payment;
use EverAccounting\Legacy\Models\Revenue;
...
function eaccounting_get_payment( $payment ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Payment( $payment );
 ...
 *
 * @return EverAccounting\Legacy\Models\Payment|\WP_Error|bool
 * @since 1.1.0
 */
...
function eaccounting_delete_payment( $payment_id ) {
	try {
		$payment = new EverAccounting\Legacy\Models\Payment( $payment_id );
...
function eaccounting_get_revenue( $revenue ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Revenue( $revenue );
 ...
 * @param bool  $wp_error Optional. Whether to return a WP_Error on failure. Default false.
 *
 * @return EverAccounting\Legacy\Models\Revenue|\WP_Error|bool
 * @since 1.1.0
 */
...
function eaccounting_delete_revenue( $revenue_id ) {
	try {
		$revenue = new EverAccounting\Legacy\Models\Revenue( $revenue_id );
 ...
 *
 * @return \EverAccounting\Legacy\Models\Transfer|null
 * @since 1.1.0
 */
function eaccounting_get_transfer( $transfer ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Transfer( $transfer );
 ...
 * @param bool  $wp_error Optional. Whether to return a WP_Error on failure. Default false.
 *
 * @return \EverAccounting\Legacy\Models\Transfer|\WP_Error|\bool
...
function eaccounting_insert_transfer( $args, $wp_error = true ) {
	...
	try {
		...
		$item = new \EverAccounting\Legacy\Models\Transfer( $args['id'] );
...
function eaccounting_delete_transfer( $transfer_id ) {
	try {
		$transfer = new EverAccounting\Legacy\Models\Transfer( $transfer_id );
...
function eaccounting_get_transfers( $args = array() ) {
	...
	$table        = \EverAccounting\Legacy\Repositories\Transfers::TABLE;
	$columns      = \EverAccounting\Legacy\Repositories\Transfers::get_columns();
...
function eaccounting_get_transactions( $args = array() ) {
	...
	$table        = \EverAccounting\Legacy\Repositories\Transactions::TABLE;
	$columns      = \EverAccounting\Legacy\Repositories\Transactions::get_columns();
	$qv['fields'] = wp_parse_list( $qv['fields'] );
	foreach ( $qv['fields'] as $index => $field ) {
		if ( ! in_array( $field, $columns, true ) ) {
			unset( $qv['fields'][ $index ] );
