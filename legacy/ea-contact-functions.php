 ...
 *
 * @return \EverAccounting\Legacy\Models\Customer|null
 * @since 1.1.0
 */
function eaccounting_get_customer( $customer ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Customer( $customer );
 ...
 *
 * @return EverAccounting\Legacy\Models\Customer|\WP_Error|bool
 * @since 1.1.0
 */
function eaccounting_insert_customer( $args, $wp_error = true ) {
	...
	try {
		...
		$item = new \EverAccounting\Legacy\Models\Customer( $args['id'] );
...
function eaccounting_delete_customer( $customer_id ) {
	try {
		$customer = new EverAccounting\Legacy\Models\Customer( $customer_id );
 ...
 *
 * @return \EverAccounting\Legacy\Models\Vendor|null
 * @since 1.1.0
 */
function eaccounting_get_vendor( $vendor ) {
	...
	try {
		$result = new EverAccounting\Legacy\Models\Vendor( $vendor );
 ...
 *
 * @return \EverAccounting\Legacy\Models\Vendor
 */
 ...
 * @param bool  $wp_error Optional. Whether to return a WP_Error on failure. Default false.
 *
 * @return EverAccounting\Legacy\Models\Vendor|\WP_Error|bool
 * @since 1.1.0
 */
function eaccounting_insert_vendor( $args, $wp_error = true ) {
	...
	try {
		...
		// Retrieve the vendor.
		$item = new \EverAccounting\Legacy\Models\Vendor( $args['id'] );
...
function eaccounting_delete_vendor( $vendor_id ) {
	try {
		$vendor = new EverAccounting\Legacy\Models\Vendor( $vendor_id );
...
function eaccounting_get_contacts( $args = array() ) {
	...
	$table        = \EverAccounting\Legacy\Repositories\Contacts::TABLE;
	$columns      = \EverAccounting\Legacy\Repositories\Contacts::get_columns();
