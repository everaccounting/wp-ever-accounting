...
class REST_Currencies_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		...
		$items = Currencies::query( $args );
		$total = Currencies::query( $args, true );
	...
	public function get_item_permissions_check( $request ) {
		$resource = Currencies::query( $request['id'] );
		if ( empty( $resource ) ) {
			return new \WP_Error(
				'rest_invalid_id',
	...
	public function get_item( $request ) {
		$item = Currencies::get( $request['id'] );
		if ( empty( $item ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'wp-ever-accounting' ),
	...
	public function create_item( $request ) {
		...
		$item = Currencies::insert( wp_slash( (array) $props ) );
	...
	public function update_item_permissions_check( $request ) {
		$currency = Currencies::get( $request['id'] );
		if ( empty( $currency ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'wp-ever-accounting' ),
