...
/**
 * Class REST_Currencies_Controller
 * @package EverAccounting\REST
 */
class REST_Currencies_Controller extends REST_Controller {
	/**
	 * Route base.
	 *
	 * @since 1.1.0
	...
	public function get_items( $request ) {
		...
		$items = Currencies::get_currencies( $args );
		$total = Currencies::get_currencies( $args, true );
	...
	public function get_item_permissions_check( $request ) {
		$resource = Currencies::get_currencies( $request['id'] );
		if ( empty( $resource ) ) {
			return new \WP_Error(
				'rest_invalid_id',
	...
	public function get_item( $request ) {
		$item = Currencies::get_currency( $request['id'] );
		if ( empty( $item ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'wp-ever-accounting' ),
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new \WP_Error(
				...
				__( 'Cannot create existing currecy.', 'wp-ever-accounting' ),
				array( 'status' => 400 )
			);
		}
		...
		$item = Currencies::insert_currency( wp_slash( (array) $props ) );
	...
	public function update_item_permissions_check( $request ) {
		$currency = Currencies::get_currency( $request['id'] );
		if ( empty( $currency ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'wp-ever-accounting' ),
