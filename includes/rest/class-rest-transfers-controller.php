 ...
 */
class REST_Transfers_Controller extends REST_Controller {
	/**
	 * Route base.
	 *
	 * @since 1.1.0
	...
	public function get_items( $request ) {
		...
		$items = Transfers::get_transfers( $args );
		$total = Transfers::get_transfers( $args, true );
	...
	public function get_item_permissions_check( $request ) {
		$resource = Transfers::get_transfer( $request['id'] );
		if ( empty( $resource ) ) {
			return new \WP_Error(
				'rest_invalid_id',
	...
	public function get_item( $request ) {
		$item = Transfers::get_transfer( $request['id'] );
		if ( empty( $item ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'text-domain' ),
	...
	public function create_item( $request ) {
		...
		$item = Transfers::insert_transfer( wp_slash( (array) $props ) );
	...
	public function update_item_permissions_check( $request ) {
		$transfer = Transfers::get_transfer( $request['id'] );
		if ( empty( $transfer ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'text-domain' ),
	...
	public function prepare_item_for_response( $item, $request ) {
		$data        = $item->get_data();
		$format_date = array( 'date', 'date_created' );
		// Format date values.
	...
	protected function prepare_item_for_database( $request ) {
		...
		foreach ( $data_keys as $key ) {
			...
			// @todo implement the callable
			if ( is_array( $value ) && isset( $value['id'] ) ) {

				$props["{$key}_id"] = $value['id'];
			}
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'date' => array(
					'description' => __( 'Transfer date of the transaction.', 'wp-ever-accounting' ),
					'type'        => 'string',
					//'format'      => 'date-time',
