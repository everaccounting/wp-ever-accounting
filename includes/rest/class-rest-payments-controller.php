...
/**
 * Class REST_Payments_Controller
 *
 * @since   1.1.2
 *
 * @package Ever_Accounting\REST
 */
class REST_Payments_Controller extends REST_Controller {
	/**
	 * Route base.
	 *
	 * @since   1.1.2
	...
	public function get_items( $request ) {
		...
		$items = Transactions::query_payments( $args );
	...
	public function get_item_permissions_check( $request ) {
		$resource = Transactions::get_payment( $request['id'] );
		if ( empty( $resource ) ) {
			return new \WP_Error(
				'rest_invalid_id',
	...
	public function get_item( $request ) {
		$item = Transactions::get_payment( $request['id'] );
		if ( empty( $item ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'wp-ever-accounting' ),
	...
	public function create_item( $request ) {
		...
		$item = Transactions::insert_payment( wp_slash( (array) $props ) );
	...
	public function update_item_permissions_check( $request ) {
		$revenue = Transactions::get_payment( $request['id'] );
		if ( empty( $revenue ) ) {
			return new \WP_Error(
				'rest_invalid_id',
				__( 'Invalid ID.', 'wp-ever-accounting' ),
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$data['account'] = $account->exists() ? $account->get_data() : null;
	...
	protected function prepare_item_for_database( $request ) {
		...
		foreach ( $data_keys as $key ) {
			...
			if ( ! is_null( $value ) ) {
				switch ( $key ) {
					case 'currency':
						if ( ! empty( $request['currency'] ) && isset( $request['currency']['code'] ) && is_callable( array( Payment::class, 'set_currency_code' ) ) ) {
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'creator'          => array(
					'description' => __( 'Creator of the payment.', 'wp-ever-accounting' ),
					'type'        => 'object',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
					'properties'  => array(
