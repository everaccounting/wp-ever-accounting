...
use EverAccounting\Transfer_Query;

defined( 'ABSPATH' ) || die();

/**
 * Transfers controller class
 */
class Transfers_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		...
		$args = apply_filters( 'eaccounting_rest_transfer_query', $args, $request );

		$transfer_query = new Transfer_Query( $args );
		$query_result   = $transfer_query->get_results();
		$query_total    = $transfer_query->get_total();

		$transfers = array();

		foreach ( $query_result as $transfer ) {
		...
		$response = rest_ensure_response( $transfers );
	...
	public function get_item_permissions_check( $request ) {
		$transfer = eaccounting_get_transfer( $request['id'] );
		if ( empty( $transfer ) ) {
			return new \WP_Error(
	...
	public function create_item( $request ) {
		...
		$transfer = eaccounting_insert_transfer( wp_slash( (array) $prepared_transfer ) );
	...
	public function prepare_item_for_response( $transfer, $request ) {
		 ...
		 * @param \WP_REST_Request $request Request object.
		 */
		return apply_filters( 'eaccounting_rest_prepare_transfer', $response, $transfer, $request );
	}
	...
	protected function prepare_item_for_database( $request ) {
			...
			if ( ! is_null( $value ) ) {
				switch ( $key ) {
					case 'from_account':
						...
						if ( $account ) {
							$props['from_account_id'] = $account->id;
						}
						break;
					case 'to_account':
						if ( ! is_array( $value ) || empty( $value ) ) {
						...
						$account = eaccounting_get_account( $value['id'] );
						if ( $account ) {
							$props['to_account_id'] = $account->id;
						}
						break;
					default:
						$props[ $key ] = $value;
		 ...
		 * @param \WP_REST_Request $request Request object.
		 */
		return apply_filters( 'eaccounting_rest_pre_insert_transfer', $props, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'amount'         => array(
					'description' => __( 'Amount of the transfer.', 'wp-ever-accounting' ),
					'type'        => 'integer',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'payment_method' => array(
					'description' => __( 'Payment method of the transfer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'income_id'      => array(
					'description' => __( 'Income ID of the transfer.', 'wp-ever-accounting' ),
					'type'        => 'integer',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'expense_id'     => array(
					'description' => __( 'Expense ID of the transfer.', 'wp-ever-accounting' ),
					'type'        => 'integer',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
		...
		/**
		 * Filters the transfer's schema.
		 *
		 * @param array $schema Item schema data.
		 *
		 * @since 1.2.1
		 */
		$schema = apply_filters( 'eaccounting_rest_transfer_item_schema', $schema );
	...
	public function get_collection_params() {
		...
		/**
		 * Filter collection parameters for the transfer controller.
		 *
		 * @param array $query_params JSON Schema-formatted collection parameters.
		 *
		 * @since 1.2.1
		 */
		return apply_filters( 'eaccounting_rest_transfer_collection_params', $params );
	}
}
