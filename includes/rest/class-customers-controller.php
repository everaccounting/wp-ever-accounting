...
class Customers_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		...
		$args = apply_filters( 'ever_accounting_rest_customer_query', $request->get_params(), $request );

		$customers = Contacts::query_customers( $args );
		$total      = Contacts::query_customers( $args, true );
		...
		if ( $page > $max_pages && $total > 0 ) {
			return new \WP_Error(
				'rest_customer_invalid_page_number',
	...
	public function get_item_permissions_check( $request ) {
		$customer = Contacts::get_customer( $request['id'] );
		if ( empty( $customer ) ) {
			return new \WP_Error(
				'rest_customer_invalid_id',
				__( 'Invalid customer id..', 'wp-ever-accounting' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! current_user_can( 'ea_manage_customer' ) ) {
			return new \WP_Error(
				'rest_forbidden_context',
				__( 'Sorry, you are not allowed to edit customer.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function create_item( $request ) {
		...
		$customer = Contacts::insert_customer( wp_slash( (array) $prepared_customer ) );

		if ( is_wp_error( $customer ) ) {

			if ( 'db_insert_error' === $customer->get_error_code() ) {
				$customer->add_data( array( 'status' => 500 ) );
			} else {
				$customer->add_data( array( 'status' => 400 ) );
			}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'ea_manage_customer' ) ) {
			return new \WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to edit this customer.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'ea_manage_customer' ) ) {
			return new \WP_Error(
				'rest_cannot_delete',
				__( 'Sorry, you are not allowed to delete customer.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $customer->delete() ) {
			return new \WP_Error( 'rest_cannot_delete', __( 'The customer cannot be deleted.', 'wp-ever-accounting' ), array( 'status' => 500 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'name'         => array(
					'description' => __( 'Name for the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'view', 'embed', 'edit' ),
					'default'     => '',
					'arg_options' => array(
				...
				'company'      => array(
					'description' => __( 'Company for the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'view', 'embed', 'edit' ),
					'default'     => '',
					'arg_options' => array(
				...
				'email'        => array(
					'description' => __( 'The email address for the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
				...
				'phone'        => array(
					'description' => __( 'Phone number for the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'website'      => array(
					'description' => __( 'website of the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'format'      => 'uri',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
				...
				'vat_number'   => array(
					'description' => __( 'Vat number of the customer', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'view', 'embed', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'street'       => array(
					'description' => __( 'Street Address of the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'state'        => array(
					'description' => __( 'State Address of the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'postcode'     => array(
					'description' => __( 'Postcode of the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
				...
				'country'      => array(
					'description' => __( 'Country of the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'arg_options' => array(
				...
				'thumbnail'    => array(
					'description' => __( 'Thumbnail of the customer', 'wp-ever-accounting' ),
					'type'        => 'object',
					'context'     => array( 'embed', 'view', 'edit' ),
					'properties'  => array(
						'id'  => array(
				...
				'enabled'      => array(
					'description' => __( 'Status of the customer.', 'wp-ever-accounting' ),
					'type'        => 'boolean',
					'context'     => array( 'embed', 'view', 'edit' ),
				),
				'creator'      => array(
					'description' => __( 'Creator of the customer.', 'wp-ever-accounting' ),
					'type'        => 'object',
				...
				'date_created' => array(
					'description' => __( 'Created date of the customer.', 'wp-ever-accounting' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view' ),
					'readonly'    => true,
