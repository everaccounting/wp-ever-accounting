...
class Categories_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		...
		$args = apply_filters( 'ever_accounting_rest_category_query', $request->get_params(), $request );

		$categories = Categories::query( $args );
		$total      = Categories::query( $args, true );
	...
	public function get_item_permissions_check( $request ) {
		$Category = Categories::get( $request['id'] );
		if ( empty( $Category ) ) {
			return new \WP_Error(
	...
	public function create_item( $request ) {
		...
		$category = Categories::insert( wp_slash( (array) $prepared_category ) );
		...
		$schema = array(
			...
			'properties' => array(
				...
				'type'         => array(
					...
					'context'     => array( 'view', 'embed', 'edit' ),
					'required'    => true,
					'enum'        => array_keys( Categories::get_types() ),
					'arg_options' => array(
						'sanitize_callback' => 'sanitize_text_field',
					),
				),
