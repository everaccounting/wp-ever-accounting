...
use EverAccounting\Category_Query;
...
class Categories_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		...
		$args = apply_filters( 'eaccounting_rest_category_query', $args, $request );

		$category_query = new Category_Query( $args );
		$query_result   = $category_query->get_results();
		$query_total    = $category_query->get_total();

		$categories = array();

		foreach ( $query_result as $category ) {
			$data         = $this->prepare_item_for_response( $category, $request );
			$categories[] = $this->prepare_response_for_collection( $data );
		}

		$page      = (int) $category_query->query_vars['paged'];
		$max_pages = ceil( $query_total / (int) $category_query->query_vars['number'] );
	...
	public function get_item_permissions_check( $request ) {
		$category = eaccounting_get_category( $request['id'] );
		if ( empty( $category ) ) {
			return new \WP_Error(
				'rest_category_invalid_id',
				__( 'Invalid category id..', 'wp-ever-accounting' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! current_user_can( 'ea_manage_category' ) ) {
			return new \WP_Error(
				'rest_forbidden_context',
				__( 'Sorry, you are not allowed to edit categories.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function create_item( $request ) {
		...
		$prepared_category = $this->prepare_item_for_database( $request );

		if ( is_wp_error( $prepared_category ) ) {
			return $prepared_category;
		}

		$category = eaccounting_insert_category( wp_slash( (array) $prepared_category ) );

		if ( is_wp_error( $category ) ) {

			if ( 'db_insert_error' === $category->get_error_code() ) {
				$category->add_data( array( 'status' => 500 ) );
			} else {
				$category->add_data( array( 'status' => 400 ) );
			}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'ea_manage_category' ) ) {
			return new \WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to edit this category.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'ea_manage_category' ) ) {
			return new \WP_Error(
				'rest_cannot_delete',
				__( 'Sorry, you are not allowed to delete category.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function prepare_item_for_response( $category, $request ) {
		 ...
		 * @param \WP_REST_Request $request Request object.
		 */
		return apply_filters( 'eaccounting_rest_prepare_category', $response, $category, $request );
	}
	...
	protected function prepare_item_for_database( $request ) {
		 ...
		 * @param \WP_REST_Request $request Request object.
		 */
		return apply_filters( 'eaccounting_rest_pre_insert_category', $props, $request );
	}
	...
	public function get_item_schema() {
		 ...
		 * @since 1.2.1
		 */
		$schema = apply_filters( 'eaccounting_rest_category_item_schema', $schema );
	...
	public function get_collection_params() {
		...
		/**
		 * Filter collection parameters for the category controller.
		 *
		 * @param array $query_params JSON Schema-formatted collection parameters.
		 *
		 * @since 1.2.1
		 */
		return apply_filters( 'eaccounting_rest_category_collection_params', $params );
	}
}
