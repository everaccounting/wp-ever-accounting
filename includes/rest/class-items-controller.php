...
class Items_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		...
		$args = apply_filters( 'ever_accounting_rest_item_query', $request->get_params(), $request );

		$products = Items::query( $args );
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'ea_manage_item' ) ) {
			return new \WP_Error(
				'rest_forbidden_context',
				__( 'Sorry, you are not allowed to get this item.', 'wp-ever-accounting' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function create_item( $request ) {
		...
		$item = Items::insert( wp_slash( (array) $prepared_item ) );
