		...
		if ( false === $total ) {
			...
			if ( ! empty( $total ) ) {
				...
				$revenues    = $wpdb->get_results(
					$wpdb->prepare( "SELECT Sum(amount) amount, currency_code, currency_rate FROM   {$wpdb->prefix}ea_transactionsWHERE  type = %s AND document_id IN ($invoice_ids) GROUP  BY currency_code,currency_rate", 'income' ) // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				);
