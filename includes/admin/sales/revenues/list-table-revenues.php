...
class List_Table_Revenues extends List_Table {
	...
	function column_date( $revenue ) {
		...
		$value = sprintf( '<a href="%1$s">%2$s</a>',
			esc_url( eaccounting_admin_url( [ 'action' => 'edit', 'account_id' => $revenue->get_id() ] ) ),
			$date
		);

		return apply_filters( 'eaccounting_revenue_table_date', $value, $revenue );
	}
	...
	function column_amount( $revenue ) {
		return apply_filters( 'eaccounting_revenue_table_amount', $revenue->get_formatted_amount(), $revenue );
	}
	...
	function column_account_id( $revenue ) {
		...
		return apply_filters( 'eaccounting_revenue_table_account', esc_html( $name ), $revenue );
	}
	...
	function column_category_id( $revenue ) {
		...
		return apply_filters( 'eaccounting_revenue_table_category', esc_html( $name ), $revenue );
	}
	...
	function column_reference( $revenue ) {
		...
		return apply_filters( 'eaccounting_revenue_table_reference', esc_html( $reference ), $revenue );
	}
