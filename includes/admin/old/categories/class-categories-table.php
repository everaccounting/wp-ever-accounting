...
class EAccounting_Categories_Table extends WP_List_Table {
	/**
	...
	public function get_views() {
		$base = admin_url( 'admin.php?page=eaccounting-categories' );
	...
	function column_name( $item ) {
		...
		if ( strtolower( $item->status ) == '1' ) {
			$row_actions['deactivate'] = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array(
					...
					'account'            => $item->id
				), $base ), 'eaccounting_categories_nonce' ) ) . '">' . __( 'Deactivate', 'wp-ever-accounting' ) . '</a>';
			...
			$row_actions['activate'] = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array(
					...
					'account'            => $item->id
				), $base ), 'eaccounting_categories_nonce' ) ) . '">' . __( 'Activate', 'wp-ever-accounting' ) . '</a>';
		}

		$row_actions['delete'] = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array(
				'eaccounting_action' => 'delete_product',
				'account'            => $item->id
			), $base ), 'eaccounting_categories_nonce' ) ) . '">' . __( 'Delete', 'wp-ever-accounting' ) . '</a>';
	 ...
	 */
	function no_items() {
		_e( 'No Products found.', 'wp-ever-accounting' );
	}
	...
	public function get_results() {
		...
		$this->active_count   = eaccounting_get_categories( array_merge( $args, array( 'status' => '1' ) ), true );
		$this->inactive_count = eaccounting_get_categories( array_merge( $args, array( 'status' => '0' ) ), true );
