...
function eaccounting_products_page() {
	...
	echo eaccounting()->notices->show();
	if ( isset( $_GET['eaccounting-action'] ) && $_GET['eaccounting-action'] == 'edit_product' ) {
		require_once dirname( __FILE__ ) . '/edit-product.php';
	} elseif ( isset( $_GET['eaccounting-action'] ) && $_GET['eaccounting-action'] == 'add_product' ) {
		require_once dirname( __FILE__ ) . '/edit-product.php';
	} else {
		require_once dirname( __FILE__ ) . '/class-products-table.php';
		$products_table = new EAccounting_Products_Table();
		$products_table->process_bulk_action();
		...
		<a href="<?php echo esc_url( add_query_arg( array( 'eaccounting-action' => 'add_product' ), $base_url ) ); ?>" class="page-title-action"><?php _e( 'Add New', 'wp-ever-accounting' ); ?></a>
