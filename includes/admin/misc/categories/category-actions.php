...
function eaccounting_activate_category( $data ) {
	...
	wp_redirect( admin_url( 'admin.php?page=eaccounting-misc&tab=categories&notice=1' ) );
}
...
function eaccounting_deactivate_category( $data ) {
	...
	wp_redirect( admin_url( 'admin.php?page=eaccounting-misc&tab=categories&notice=1' ) );
	error_log( 'eaccounting_deactivate_category' );
}
...
function eaccounting_edit_category( $posted ) {
	...
	wp_redirect(add_query_arg(['notice' => '1', 'eaccounting-action' => 'edit_category', 'category' => $created]));
}
