...
class Admin_Notices {
	...
	public static function init() {
		self::$notices = get_option( 'ever_accounting_admin_notices', array() );
		add_filter( 'wp_redirect', array( __CLASS__, 'redirect' ), 1 );
		add_action( 'admin_notices', array( __CLASS__, 'render_notices' ), 15 );
		add_action( 'wp_ajax_dismiss_admin_notice_' . EVER_ACCOUNTING_BASENAME, array( __CLASS__, 'handle_dismiss_notice' ) );
	}
	...
	public static function save_notices() {
		update_option( 'ever_accounting_admin_notices', self::$notices, true );
	}
	...
	public static function render_notice( $notice ) {
		$output            = '';
		$plugin_class      = EVER_ACCOUNTING_SLUG;
		$dismissible_class = $notice['dismissible'] ? 'is-dismissible' : '';
		$nonce             = $notice['dismissible'] ? wp_create_nonce( EVER_ACCOUNTING_SLUG . '_dismiss_notice' ) : '';
		...
		$output .= sprintf(
			...
			esc_attr( $notice['id'] ),
			esc_attr( EVER_ACCOUNTING_BASENAME ),
			esc_attr( $plugin_class ),
			esc_attr( $nonce )
		);
		echo wp_kses_post( $output );

		if ( ! did_action( 'wp_ever_accounting_render_admin_notice_js' ) ) {
			self::render_admin_notice_js();
			do_action( 'wp_ever_accounting_render_admin_notice_js' );
		}
	}
	...
	public static function handle_dismiss_notice() {
		...
		$nonce_action = EVER_ACCOUNTING_SLUG . '_dismiss_notice';

		if ( empty( $notice_id ) || EVER_ACCOUNTING_BASENAME !== $plugin_id || ! wp_verify_nonce( $nonce, $nonce_action ) ) {
			wp_send_json_error( esc_html__( 'Something does not look appropriate.', 'wp-ever-accounting' ) );
		}
