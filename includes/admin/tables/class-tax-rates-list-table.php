...
class EAccounting_Rax_Rates_List_Table extends EAccounting_List_Table {
	/**
	...
	function column_name( $item ) {
		$tax_rate_url    = add_query_arg( array( 'tax_rate' => $item->id ), $this->base_url );
		$edit_url       = wp_nonce_url( add_query_arg( [ 'eaccounting-action' => 'edit_tax_rate' ], $tax_rate_url ), 'eaccounting_tax_rates_nonce' );
		$activate_url   = wp_nonce_url( add_query_arg( [ 'eaccounting-action' => 'activate_tax_rate' ], $tax_rate_url ), 'eaccounting_tax_rates_nonce' );
		$deactivate_url = wp_nonce_url( add_query_arg( [ 'eaccounting-action' => 'deactivate_tax_rate' ], $tax_rate_url ), 'eaccounting_tax_rates_nonce' );
		$delete_url     = wp_nonce_url( add_query_arg( [ 'eaccounting-action' => 'delete_tax_rate' ], $tax_rate_url ), 'eaccounting_tax_rates_nonce' );
	...
	public function process_bulk_action() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-tax_rates' ) ) {
			return;
		}
		...
		foreach ( $ids as $id ) {
			...
			if ( 'activate' === $this->current_action() ) {
				eaccounting_insert_tax_rate( [ 'id' => $id, 'status' => '1' ] );
			}
			if ( 'deactivate' === $this->current_action() ) {
				eaccounting_insert_tax_rate( [ 'id' => $id, 'status' => '0' ] );
			}
		}
	}
