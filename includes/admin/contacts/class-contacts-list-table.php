...
class EAccounting_Contacts_List_Table extends EAccounting_List_Table {
	...
	public function get_columns() {
		$columns = array(
			...
			'contact' => __( 'Contact', 'wp-eaccounting' ),
			'address' => __( 'address', 'wp-eaccounting' ),
	...
	function column_name( $item ) {
		$contact_url    = add_query_arg( array( 'contact' => $item->id ), $this->base_url );
		...
		return sprintf( '<strong><a href="%1$s">%2$s</a></strong>', $edit_url, stripslashes( eaccounting_get_contact_name($item) ) ) . $this->row_actions( $row_actions );
	}
