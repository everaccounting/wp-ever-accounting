...
//		}
	...
	public function extra_tablenav( $which = '' ) {
		...
		$selected_type  = isset( $_GET['type'] ) && array_key_exists( $_GET['type'], $category_types ) ? sanitize_key( $_GET['type'] ) : '';
		?>
		<div class="alignleft actions bulkactions">
			<select name="type" id="category-type">
				<option value=""><?php _e( 'All Types', 'wp-ever-accounting' ); ?></option>
				<?php foreach ( $category_types as $type => $label ) : ?>
					<option value="<?php echo esc_attr( $type ); ?>"<?php eaccounting_selected( $selected_type, $category_types ); ?>><?php echo esc_html( $label ); ?></option>
				<?php endforeach; ?>
			</select>
		</div>
		...
		submit_button( __( 'Filter', 'wp-ever-accounting' ), 'action', 'filter_action', false );
		echo "\n";
	}
	...
	public function prepare_items() {
		...
		$this->items = Query_Category::init()->wp_query( $args )->get( OBJECT, 'eaccounting_get_category' );

		$this->active_count = Query_Category::init()->wp_query( array_merge( $this->query_args, array(
			'status' => 'active',
			'search' => $search
		) ) )->count();

		$this->inactive_count = Query_Category::init()->wp_query( array_merge( $this->query_args, array(
			'status' => 'inactive',
			'search' => $search
		) ) )->count();
