...
function eaccounting_action_edit_product( $data ) {
	...
	$args = array(
		...
		'status'         => eaccounting_sanitize_status( $data['status'] ),
	);
	...
	eaccounting_admin_notice( sprintf( __( 'Product <strong>%s</strong> saved successfully', 'wp-ever-accounting' ), $args['name'] ) );
