...
class Transaction_Query {
	...
	/**
	 * List of transactions located by the query.
	 *
	 * @since 1.2.1
	 * @var array
	 */
	...
	/**
	 * The number of transactions found for the current query.
	 *
	 * @since 1.2.1
	 * @var int
	 */
	...
	public function prepare_query( $query = null ) {
		 ...
		 *
		 */
		do_action_ref_array( 'eaccounting_parse_transaction_query', array( &$this ) );
		 ...
		 *
		 */
		$qv = apply_filters( 'eaccounting_get_transactions_args', $qv );
		...
		if ( $qv['orderby'] === 'amount' || ! empty( $qv['amount_min'] ) || ! empty( $qv['amount_max'] ) || ! empty( $qv['amount_between'] ) ) {
			$query_join .= "LEFT OUTER JOIN (select id, amount/currency_rate as default_amount from $this->table ) as t2 ON (t2.id = $this->table.id) ";
		}
		...
		if ( ! empty( $qv['amount_min'] ) ) {
			$query_where .= $wpdb->prepare( " AND t2.default_amount >= (%f)", (float) $qv['amount_min'] );
		}

		if ( ! empty( $qv['amount_max'] ) ) {
			$query_where .= $wpdb->prepare( " AND t2.default_amount <= (%f)", (float) $qv['amount_max'] );
		}

		if ( ! empty( $qv['amount_between'] ) && is_array( $qv['amount_between'] ) ) {
			$min         = min( $qv['amount_between'] );
			$max         = max( $qv['amount_between'] );
			$query_where .= $wpdb->prepare( " AND t2.default_amount >= (%f) AND t2.default_amount <= (%f) ", (float) $min, (float) $max );
		}
	...
	public function query() {
		 ...
		 *
		 */
		$clauses = (array) apply_filters_ref_array( 'eaccounting_transaction_query_clauses', array( $this->sql_clauses, &$this ) );
		 ...
		 *
		 */
		$this->results = apply_filters_ref_array( 'eaccounting_pre_transaction_query', array( null, &$this ) );
		...
		if ( null === $this->results ) {
			...
			/**
			 * Filters the raw transaction results array.
			 *
			 * @param Category[] $transactions Array of transactions objects.
			 * @param Transaction_Query $query The Transaction_Query instance (passed by reference).
			 *
			 * @since 1.2.1
			 *
			 */
			$this->results = apply_filters_ref_array( 'eaccounting_transactions_results', array( $this->results, &$this ) );
	...
	/**
	 * Return the total number of transactions for the current query.
	 *
