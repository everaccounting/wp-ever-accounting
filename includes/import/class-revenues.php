...
class Revenues extends CSV_Importer {
	...
	protected function import_item( $data ) {
		...
		$currency = new \EverAccounting\Currency( $data['currency_code'] );
		...
		$customer    = ( '' != $data['customer_name'] ) ? eaccounting_get_contacts( array( 'search' => $data['customer_name'], 'search_cols' => array( 'name' ), 'type' => 'customer' ) ) : '';
