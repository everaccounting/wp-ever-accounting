...
class EAccounting_Reports_Controller extends EAccounting_REST_Controller {
	...
	public function get_summery( $request ) {
		...
		$income_query = "SELECT SUM(amount/currency_rate) from $wpdb->ea_revenues WHERE category_id IN (SELECT id FROM $wpdb->ea_categories WHERE type='income')";
		if ( ! empty( $duration['start'] ) ) {
			$income_query .= $wpdb->prepare( " AND paid_at >= DATE(%s)", $duration['start'] );
		}

		if ( ! empty( $duration['end'] ) ) {
			$income_query .= $wpdb->prepare( " AND paid_at <= DATE(%s)", $duration['end'] );
		}

		$income = $wpdb->get_var( $income_query );

		$expense_query = "SELECT SUM(amount/currency_rate) from $wpdb->ea_payments WHERE category_id IN (SELECT id FROM $wpdb->ea_categories WHERE type='expense')";
		if ( ! empty( $duration['start'] ) ) {
			$expense_query .= $wpdb->prepare( " AND paid_at >= DATE(%s)", $duration['start'] );
		}

		if ( ! empty( $duration['end'] ) ) {
			$expense_query .= $wpdb->prepare( " AND paid_at <= DATE(%s)", $duration['end'] );
		}

		$expense = $wpdb->get_var( $expense_query );
		...
		if ( ! is_wp_error( $default_currency ) ) {
			$response = array(
				'income'  => eaccounting_format_price( $income, $default_currency->code ),
				'expense' => eaccounting_format_price( $expense, $default_currency->code ),
				'profit'  => eaccounting_format_price( $profit, $default_currency->code ),
			);
		}
	...
	public function get_cashflow( $request ) {
		...
		$start_date = eaccounting_sanitize_date( $duration['start'], 'Y-m-d', $year_start );
		$end_date   = eaccounting_sanitize_date( $duration['end'], 'Y-m-d', date( 'Y-m-d' ) );
