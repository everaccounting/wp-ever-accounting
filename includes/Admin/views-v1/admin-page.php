...
$tabs         = apply_filters( 'eac_admin_' . $page_hook . '_tabs', $tabs );
	...
	<div class="wrap eac-wrap">
		<?php if ( ! empty( $tabs ) && count( $tabs ) > 1 ) : ?>
			<nav class="nav-tab-wrapper eac-navbar">
				 ...
				 * @since 1.0.0
				 */
				do_action( 'ever_accounting_admin_' . $page_hook . '_nav_items', $current_tab, $tabs );
				?>
			</nav>
		<?php endif; ?>
		...
		if ( ! empty( $current_tab ) && $page_hook !== $current_tab ) {
			 ...
			 * @since 1.0.0
			 */
			do_action( "ever_accounting_admin_{$page_hook}_{$current_tab}", $current_tab );
		} else {
			/**
			 ...
			 * @since 1.0.0
			 */
			do_action( "ever_accounting_admin_{$page_hook}", $current_tab );
		}
		?>
	</div>
<?php
