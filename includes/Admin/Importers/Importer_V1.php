...
//	abstract protected function import_item( $data );
	...
	public function parse_text_field( $value ) {
		return eac_clean( $this->unescape_data( $value ) );
	}
	 ...
	 *
	 * By default, is applied eac_clean() to all not listed fields
	 ...
	 * @return string
	 */
	public function parse_textarea_field( $description ) {
		$parts = explode( "\\\\n", $description );
	...
	public function parse_country_field( $country ) {
		$country = eac_clean( $country );

		return array_key_exists( $country, eac_get_countries() ) ? $country : '';
	}
}
