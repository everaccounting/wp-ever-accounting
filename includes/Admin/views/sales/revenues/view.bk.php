...
$actions = array(
	array(
		'url'  => admin_url( 'admin.php?page=eac-sales&tab=payments&action=edit&payment_id=' . $revenue->get_id() ),
		'text' => __( 'Edit', 'wp-ever-accounting' ),
	),
	array(
		'url'  => wp_nonce_url( admin_url( 'admin.php?page=eac-sales&tab=payments&action=delete&payment_id=' . $revenue->get_id() ), 'eac_delete_payment' ),
...
<div class="eac-section-header margin-bottom-4">
	<div>
		<h2>
			<?php echo esc_html( $revenue->get_number() ); ?>
		</h2>
	...
	<div>
		...
		?>
		<a href="<?php echo esc_url( eac_action_url( 'action=send_payment_receipt&id=' . $revenue->get_id(), false ) ); ?>" class="button button-primary">
