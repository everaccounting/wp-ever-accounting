		...
		<div class="bkit-panel bkit-p-0">
			...
			<div class="eac-document-form__section document-info">
				<div class="document-info__left">
					...
					<div class="billing-fields tw-box-sizing:border-box tw-flex-wrap:wrap tw-gap-x-[20px] tw-grid tw-grid-cols-2 tw-justify-between tw-items-self-start tw-content-flex-start">
						<?php
						$fields = array(
							array(
								'type'          => 'text',
						...
						<?php foreach ( $columns as $key => $label ) : ?>
							...
							<?php else : ?>
								<td class="line-item__<?php echo esc_attr( $key ); ?>">
									...
									switch ( $key ) {
										case 'price':
											printf( '<input class="line-item__price-input" type="number" name="items[%s][price]" value="%s" placeholder="%s" />', esc_attr( $item_key ), esc_attr( $item->price ), esc_attr__( 'Price', 'wp-ever-accounting' ) );
											break;
										case 'quantity':
