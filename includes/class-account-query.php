...
class Account_Query {
	...
	/**
	 * The amount of found accounts for the current query.
	 *
	 * @since 1.2.1
	 * @var int
	 */
	public $found_accounts = 0;
	...
	/**
	 * Parses arguments passed to the account query with default query parameters.
	 *
	 * @param string|array $query Array or query string of account query parameters. Default empty.
	 *
	...
	public function parse_query( $query = '' ) {
		 ...
		 *
		 * @param Account_Query $this The Account_Query instance (passed by reference).
		 *
		 * @since 1.2.1
		 *
		 */
	...
	/**
	 * Sets up the WordPress query for retrieving accounts.
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 *
	 * @return array|int List of Accounts objects, a list of site IDs when 'fields' is set to 'ids', or the number of accounts when 'count' is passed as a query var.
	 * @since 1.2.1
	 *
	 */
	...
	/**
	 * Retrieves a list of accounts matching the query vars.
	 *
	 * @return array|int List of WP_Site objects, a list of site IDs when 'fields' is set to 'ids', or the number of sites when 'count' is passed as a query var.
	...
	public function get_accounts() {
		 ...
		 *
		 * @param Account_Query $this Current instance of Account_Query (passed by reference).
		 *
		 * @since 1.2.1
		 *
		 */
		...
		/**
		 * Filters the account data before the get_accounts query takes place.
		 *
		 * @param array|int|null $account_data Return an array of site data to short-circuit Account Query site query.
		 * @param Account_Query $query The Account_Query instance, passed by reference.
		 *
		 * @since 1.2.1
		 *
		 */
		$account_data = apply_filters_ref_array( 'eaccounting_accounts_pre_query', array( $account_data, &$this ) );

		if ( null !== $account_data ) {
			if ( is_array( $account_data ) && ! $this->query_vars['count'] ) {
				$this->accounts = $account_data;
			}
		...
		$cache_key   = "get_accounts:$key:$last_changed";
		$cache_value = wp_cache_get( $cache_key, 'ea_accounts' );

		if ( false === $cache_value ) {
			$account_ids = $this->get_account_ids();
			if ( $account_ids ) {
				$this->set_found_accounts();
			}

			$cache_value = array(
				'account_ids'    => $account_ids,
				'found_accounts' => $this->found_accounts,
			);
			wp_cache_add( $cache_key, $cache_value, 'ea_accounts' );
		} else {
			$account_ids          = $cache_value['account_ids'];
			$this->found_accounts = $cache_value['found_accounts'];
		}

		if ( $this->found_accounts && $this->query_vars['number'] ) {
			$this->max_num_pages = ceil( $this->found_accounts / $this->query_vars['number'] );
		}
		...
		foreach ( $account_ids as $account_id ) {
			$_account = eaccounting_get_account( $account_id );
			if ( $_account ) {
				$_accounts[] = $_account;
			}
		}
		 ...
		 *
		 * @param Account[] $_accounts An array of Account objects.
		 * @param Account $query Current instance of Account_Query (passed by reference).
		 *
		 * @since 1.2.1
		 *
		 */
		$_accounts = apply_filters_ref_array( 'eaccounting_the_accounts', array( $_accounts, &$this ) );

		// Convert to Account instances.
		$this->accounts = array_map( 'get_site', $_accounts );
