jQuery( function ( $ ) {
	...
	$.ea_modal = function ( template, options ) {
		...
		this.block = function () {
			...
			$( '.ea-modal__footer button', plugin.$modal ).attr(
				'disabled',
				'disabled'
			);
		};
		...
		this.unblock = function () {
			...
			$( '.ea-modal__footer button', plugin.$modal ).removeAttr(
				'disabled'
			);
		};
		...
		this.close = function () {
			$( document.body ).trigger( 'ea_modal_closed', [ plugin ] );
			...
			$( document.body ).trigger( 'ea_modal_removed', [ plugin ] );
		};
		...
		this.init = function () {
			...
			// $( '.ea-modal__content .ea-input-date' ).datepicker( {
			// 	dateFormat: 'yy-mm-dd',
			// } );
			...
			$( document.body ).trigger( 'ea_modal_loaded', [ plugin ] );
				...
				if ( typeof plugin.options.onSubmit === 'function' ) {
					...
					$.post( ajaxurl, data, function ( json ) {
						...
						if ( json.success ) {
							...
							$(
								document.body
							).trigger( 'ea_modal_form_submitted', [
								json,
								plugin,
								data,
							] );
					...
					$( document.body ).trigger( 'ea_modal_form_submitted', [
						plugin,
						data,
					] );
				}
