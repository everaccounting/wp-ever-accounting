...
(function ($, window, document, undefined) {
	...
	$(document).on('init-form', function () {
		// Account form.
		$('#eac-account-form :input[name="currency"]').on('change', function () {
			var currency = $(this).val();
			var $form = $(this).closest('form');
			eac_admin.input_mask($(':input[name="opening_balance"]', $form), currency);
		}).trigger('change');
		// Transfer form.
		$('#eac-transfer-form :input[name="from_account_id"]').on('change', function () {
			...
			$.post(eac_admin_vars.ajax_url, data, function (response) {
				if (response.success) {
					...
					$(':input[name="conversion_rate"]', $form).val(eac_admin_vars.currencies[currency].rate || 1);
		...
		$('#eac-transfer-form :input[name="currency"]').on('change', function () {
			var currency = $(this).val();
			var $form = $(this).closest('form');
			eac_admin.input_mask($(':input[name="amount"]', $form), currency);
		}).trigger('change');
