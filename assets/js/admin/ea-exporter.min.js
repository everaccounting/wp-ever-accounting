		...
		(i.eaccounting_exporter = function (t, e) {
			...
			return (
				(this.submit = function (t) {
					if ((t.preventDefault(), s.$submit_btn.hasClass('disabled'))) return !1;
					s.$submit_btn.addClass('disabled'),
						s.reset(),
						s.$submit_btn.parent('p').append('<span class="spinner is-active"></span>'),
						s.$form.append('<div class="ea-batch-notice"><div class="ea-batch-progress"><div></div></div></div>'),
						s.process_step(1);
				}),
					...
					window.wp.ajax.send(s.action, {
						...
						success: function (t) {
							if ('done' === t.step)
								return (
									s.reset(),
									s.$form.append(
										'<div class="ea-batch-notice"><div class="updated success"><p>' + t.message + '</p></div></div>'
									),
									(window.location = t.url),
									!1
								);
							s.$form.find('.ea-batch-progress div').animate({ width: t.percentage + '%' }, 50, function () {}),
						...
						error: function (t) {
							s.reset(),
								t.message &&
									s.$form.append(
										'<div class="ea-batch-notice"><div class="updated error"><p>' + t.message + '</p></div></div>'
									),
								console.warn(t);
						},
					});
