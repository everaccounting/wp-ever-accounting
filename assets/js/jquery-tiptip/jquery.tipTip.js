		...
		return this.each( function () {
			...
			if ( org_title != '' ) {
				...
				function active_tiptip() {
					...
					var marg_top = Math.round(
						top + org_height + opts.edgeOffset
					);
					var t_class = '';
					var arrow_top = '';
					...
					var left_compare =
						tip_w + left > parseInt( $( window ).width() );

					if (
						( right_compare && w_compare < 0 ) ||
						( t_class == '_right' && ! left_compare ) ||
						( t_class == '_left' &&
							left < tip_w + opts.edgeOffset + 5 )
					) {
						t_class = '_right';
						arrow_top = Math.round( tip_h - 13 ) / 2;
						arrow_left = -12;
						marg_left = Math.round(
							left + org_width + opts.edgeOffset
						);
