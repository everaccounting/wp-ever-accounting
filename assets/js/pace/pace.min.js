( function () {
	...
	for (
		m = {
			...
			y = function () {
				for (
					var t,
						...
						n,
						r = arguments[ 0 ],
						s = 2 <= arguments.length ? Q.call( arguments, 1 ) : [],
						o = 0,
						i = s.length;
					o < i;
					o++
			...
			$.prototype.off = function ( t, e ) {
				var n, r, s;
				if (
					null != ( null != ( r = this.bindings ) ? r[ t ] : void 0 )
				) {
				...
				if ( null != ( s = this.bindings ) && s[ a ] ) {
					for ( n = 0, i = []; n < this.bindings[ a ].length;  )
						( e = ( o = this.bindings[ a ][ n ] ).handler ),
							...
							r
								? i.push( this.bindings[ a ].splice( n, 1 ) )
								: i.push( n++ );
					return i;
				}
			},
	...
	function at( t ) {
		( this.source = t ),
			...
			null != this.source &&
				( this.progress = O( this.source, 'progress' ) );
	}
	( K = Error ),
		Y( tt, K ),
		( i = tt ),
		( et.prototype.getElement = function () {
			var t;
			if ( null == this.el ) {
				if ( ! ( t = document.querySelector( j.target ) ) )
					throw new i();
		...
		( v.ignore = function () {
			...
			return (
				P.unshift( 'ignore' ), ( t = e.apply( null, n ) ), P.shift(), t
			);
		} ),
		( v.track = function () {
			var t,
				...
				n = 2 <= arguments.length ? Q.call( arguments, 1 ) : [];
			return (
				P.unshift( 'track' ), ( t = e.apply( null, n ) ), P.shift(), t
			);
		} ),
		( _ = function ( t ) {
			var e;
			if ( ( null == t && ( t = 'GET' ), 'track' === P[ 0 ] ) )
				return 'force';
			if ( ! P.length && j.ajax ) {
				if ( 'socket' === t && j.ajax.trackWebSockets ) return ! 0;
				if (
					( ( e = t.toUpperCase() ),
					0 <= Z.call( j.ajax.trackMethods, e ) )
				)
					return ! 0;
			}
			return ! 1;
		...
		( N = function ( t ) {
			for (
				var e, n = j.ajax.ignoreURLs, r = 0, s = n.length;
				r < s;
				r++
			)
		...
		( L = function () {
			...
			if ( ! N( n ) )
				return v.running ||
					( ! 1 === j.restartOnRequestAfter && 'force' !== _( a ) )
					? void 0
					: ( ( i = arguments ),
					  'boolean' ==
							typeof ( e = j.restartOnRequestAfter || 0 ) &&
							( e = 0 ),
					  setTimeout( function () {
							var t,
								e,
								...
								o =
									'socket' === a
										? u.readyState < 2
										: 0 < ( n = u.readyState ) && n < 4;
							if ( o ) {
								for (
									v.restart(),
										s = [],
										t = 0,
										e = ( r = v.sources ).length;
									t < e;
									t++
								) {
			...
			( this.progress = 0 ),
				...
				( n = setInterval( function () {
					...
					return (
						...
						( e = d( o ) ),
						++s >= j.eventLag.minSamples &&
						e < j.eventLag.lagThreshold
		...
		( T = function () {
			...
			for (
				n = 0,
					s = ( a = null != ( i = j.extraSources ) ? i : [] ).length;
				n < s;
				n++
			)
				( F = a[ n ] ), C.push( new F( j ) );
		...
		( v.start = function ( t ) {
			...
			return document.querySelector( '.pace' )
				? ( v.trigger( 'start' ), v.go() )
				: setTimeout( v.start, 50 );
		} ),
