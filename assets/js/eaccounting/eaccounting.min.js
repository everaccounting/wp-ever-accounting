			...
			( o.eaccounting_select2 = function ( e, n ) {
				( this.el = e ),
					...
					! this.ajax ||
						...
						( console.warn(
							'ajax type defined without nonce and data type'
						),
						( this.ajax = ! 1 ) ),
					this.creatable &&
						! this.template &&
						...
						( t.ajax = {
							...
							processResults: function ( e, n ) {
								return (
									...
									{
										results: e.results,
										pagination: { more: e.pagination.more },
									}
								);
							},
						} );
					...
					.iris( {
						change: function ( e, n ) {
							c( t )
								...
								.css( { backgroundColor: n.color.toString() } );
						},
						hide: ! 0,
						border: ! 0,
					} )
			...
			( a.eaccounting_creatable = function ( e, n ) {
				...
				return (
					...
					( this.init_plugins = function () {
						a( '.ea-select2', this.$el ).eaccounting_select2(),
							a( '.ea-input-color' ).ea_color_picker();
					} ),
					( this.handleSubmit = function ( e, t ) {
						c.block(),
							t.disableSubmit(),
							'function' == typeof c.options.onSubmit &&
								c.options.onSubmit( c.$el, e, t ),
							wp.ajax.send( {
								data: e,
								success: function ( e ) {
									var n = c.options.option( e.item );
									c.$el.eaccounting_select2( {
										data: [ n ],
									} ),
										c.$el.val( n.id ).trigger( 'change' ),
										a.eaccounting_notice(
											e.message,
											'success'
										),
										t.closeModal(),
								...
								error: function ( e ) {
									a.eaccounting_notice( e.message, 'error' ),
										t.enableSubmit();
								},
							} );
					} ),
							...
							n.is( c.$el ) &&
								...
								a( this ).ea_backbone_modal( {
									...
									onReady: function ( e, n ) {
										'function' ==
											typeof c.options.onReady &&
											c.options.onReady( c.$el, n, c );
									},
								} ) );
					} ),
					( this.init = function () {
						a( document )
							.on( 'ea_trigger_creatable', c.handleModal )
							.on( 'ea_backbone_modal_loaded', c.init_plugins );
					} ),
					this.init(),
					this
				);
			...
			( c.eaccounting_form = function ( e, n ) {
				...
				( t.el = e ),
					...
					t.$el.on( 'submit', function ( e ) {
						e.preventDefault(),
							...
							wp.ajax.send( {
								...
								success: function ( e ) {
									t.unblock(),
										c.eaccounting_notice( e, 'success' ),
										c.eaccounting_redirect( e );
								},
								error: function ( e ) {
									console.warn( e ),
										t.unblock(),
										c.eaccounting_notice(
											e.message,
											'error'
										);
								},
					...
					t.code.on( 'change', function () {
						...
						try {
							( currency =
								eaccounting_i10n.global_currencies[ e ] ),
								c( '#precision', t.$el )
								...
								c( '#symbol', t.$el )
									.val( currency.symbol )
									.change(),
								c( '#decimal_separator', t.$el )
									.val( currency.decimal_separator )
									.change(),
								c( '#thousand_separator', t.$el )
