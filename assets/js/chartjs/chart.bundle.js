...
*/
	...
	function getRgba( string ) {
		...
		if ( match ) {
			...
			if ( hexAlpha ) {
				a =
					Math.round( ( parseInt( hexAlpha, 16 ) / 255 ) * 100 ) /
					100;
			}
	...
	Color.prototype.setValues = function ( space, vals ) {
		...
		for ( var sname in spaces ) {
			if ( sname !== space ) {
				values[ sname ] = colorConvert[ space ][ sname ](
					values[ space ]
				);
			}
		}
	...
	var core_animations = {
		...
		cancelAnimation: function ( chart ) {
			var index = helpers$1.findIndex( this.animations, function (
				animation
			) {
		...
		arrayEvents.forEach( function ( key ) {
			...
			Object.defineProperty( array, key, {
				...
				value: function () {
					...
					helpers$1.each( array._chartjs.listeners, function (
						object
					) {
	...
	var element_arc = core_element.extend( {
		...
		getArea: function () {
			...
			return (
				...
				( Math.pow( vm.outerRadius, 2 ) -
					Math.pow( vm.innerRadius, 2 ) )
			);
		},
	...
	var core_interaction = {
		...
		modes: {
			...
			dataset: function ( chart, e, options ) {
				...
				if ( items.length > 0 ) {
					items = chart.getDatasetMeta( items[ 0 ]._datasetIndex )
						.data;
				}
	...
	function injectCSS( rootNode, css ) {
		...
		var expando =
			rootNode[ EXPANDO_KEY ] || ( rootNode[ EXPANDO_KEY ] = {} );
	...
	var platform_dom$2 = {
		...
		addEventListener: function ( chart, type, listener ) {
			...
			var proxy = ( proxies[ chart.id + '_' + type ] = function (
				event
			) {
	...
	function createTooltipItem( element ) {
		...
		var controller = element._chart.getDatasetMeta( datasetIndex )
			.controller;
	...
	helpers$1.extend(
		...
		/** @lends Chart */ {
			...
			ensureScalesHaveIDs: function () {
				...
				helpers$1.each( scalesOptions.xAxes, function (
					xAxisOptions,
					index
				) {
					if ( ! xAxisOptions.id ) {
				...
				helpers$1.each( scalesOptions.yAxes, function (
					yAxisOptions,
					index
				) {
					if ( ! yAxisOptions.id ) {
			...
			buildOrUpdateScales: function () {
				...
				helpers$1.each( items, function ( item ) {
					...
					if ( id in scales && scales[ id ].type === scaleType ) {
						...
						var scaleClass = core_scaleService.getScaleConstructor(
							scaleType
						);
						if ( ! scaleClass ) {
							return;
			...
			updateDatasets: function () {
				...
				if (
					core_plugins.notify( me, 'beforeDatasetsUpdate' ) === false
				) {
					return;
				}
			...
			eventHandler: function ( e ) {
				...
				if (
					core_plugins.notify( me, 'beforeEvent', [ e ] ) === false
				) {
					return;
				}
	...
	var scale_category = core_scale.extend( {
		...
		getPixelForValue: function ( value, index, datasetIndex ) {
			...
			if (
				! isNullOrUndef$1( index ) &&
				! isNullOrUndef$1( datasetIndex )
			) {
	...
	var scale_logarithmic = core_scale.extend( {
		...
		_configure: function () {
			...
			me._valueRange =
				( log10( me.max ) - log10( start ) ) / ( 1 - offset );
		},
	...
	var scale_radialLinear = scale_linearbase.extend( {
		...
		determineDataLimits: function () {
			...
			helpers$1.each( chart.data.datasets, function (
				dataset,
				datasetIndex
			) {
