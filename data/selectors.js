...
import { set, map, find, get, isEmpty } from 'lodash';
...
export function hasEntities(state, name, query = {}) {
	const entities = getEntities(state, name, query);
	return Array.isArray(entities) && !isEmpty(entities);
}
...
export function hasEditsForEntity(state, name, recordId) {
	return (
		...
		Object.keys(getEntityNonTransientEdits(state, name, recordId)).length >
			0
	);
}
 ...
 */
export function isRequesting(state, selectorName, ...args) {
	return select(STORE_KEY).getIsResolving(selectorName, args);
}
