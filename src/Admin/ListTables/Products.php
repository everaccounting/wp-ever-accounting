 ...
 */
class Products extends ListTable {
	/**
	 * Get things started
	 *
	...
	public function prepare_items() {
		...
		$this->items       = eac_get_products( $args );
		$this->total_count = eac_get_products( $args, true );
	...
	public function no_items() {
		esc_html_e( 'No products found.', 'wp-ever-accounting' );
	}
	...
	public function process_bulk_action( $doaction ) {
		if ( ! empty( $doaction ) ) {
			$id  = eac_get_input_var( 'product_id' );
			$ids = eac_get_input_var( 'product_ids' );
			if ( ! empty( $id ) ) {
			...
			switch ( $doaction ) {
				case 'delete':
					$notice = __( 'Product(s) deleted successfully.', 'wp-ever-accounting' );
					break;
				case 'enable':
					$notice = __( 'Product(s) enabled successfully.', 'wp-ever-accounting' );
					break;
				case 'disable':
					$notice = __( 'Product(s) disabled successfully.', 'wp-ever-accounting' );
					break;
			}
	 ...
	 *
	 * @param Product $item The current account object.
	 *
	 * @since  1.0.2
	 * @return string Displays a checkbox.
	 */
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="product_ids[]" value="%d"/>', esc_attr( $item->get_id() ) );
	}
	...
	public function column_name( $item ) {
		$args        = array( 'product_id' => $item->get_id() );
		...
		$actions     = array(
			...
			'enable'  => sprintf( '<a href="%s">%s</a>', esc_url( wp_nonce_url( $enable_url, 'bulk-products' ) ), __( 'Enable', 'wp-ever-accounting' ) ),
			'disable' => sprintf( '<a href="%s">%s</a>', esc_url( wp_nonce_url( $disable_url, 'bulk-products' ) ), __( 'Disable', 'wp-ever-accounting' ) ),
			'delete'  => sprintf( '<a href="%s" class="del">%s</a>', esc_url( wp_nonce_url( $delete_url, 'bulk-products' ) ), __( 'Delete', 'wp-ever-accounting' ) ),
		);
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			case 'price':
				$value = esc_html( eac_format_money( $item->get_price() ) );
				break;
			case 'unit':
				$unit  = $item->get_unit();
