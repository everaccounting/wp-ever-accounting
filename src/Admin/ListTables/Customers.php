...
class Customers extends ListTable {
	...
	public function prepare_items() {
		...
		$this->items       = eac_get_customers( $args );
		$this->total_count = eac_get_customers( $args, true );
	 ...
	 *
	 * @param Customer $item The current account object.
	 *
	 * @since  1.0.2
	 * @return string Displays a checkbox.
	 */
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="customer_ids[]" value="%d"/>', esc_attr( $item->get_id() ) );
	}

	/**
	 * Renders the name column in the customers list table.
	 *
	 * @param Customer $item The current account object.
	 *
	 * @since  1.0.2
