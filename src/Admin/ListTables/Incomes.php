 ...
 */
class Incomes extends ListTable {
	/**
	 * Get things started
	 *
	...
	public function prepare_items() {
		...
		$this->items       = eac_get_incomes( $args );
		$this->total_count = eac_get_incomes( $args, true );
	...
	public function no_items() {
		esc_html_e( 'No incomes found.', 'wp-ever-accounting' );
	}
	...
	public function process_bulk_action( $doaction ) {
		if ( ! empty( $doaction ) ) {
			$id  = eac_get_input_var( 'income_id' );
			$ids = eac_get_input_var( 'income_ids', array() );
			if ( ! empty( $id ) ) {
			...
			switch ( $doaction ) {
				case 'delete':
					$notice = __( 'Income(s) deleted successfully.', 'wp-ever-accounting' );
					break;
			}
	 ...
	 *
	 * @param Income $item The current account object.
	 *
	 * @since  1.0.2
	 * @return string Displays a checkbox.
	 */
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="income_ids[]" value="%d"/>', esc_attr( $item->get_id() ) );
	}
	...
	public function column_date( $item ) {
		$args       = array( 'income_id' => $item->get_id() );
