...
class PaymentsTable extends ListTable {
	...
	public function prepare_items() {
		...
		$per_page              = $this->get_items_per_page( 'eac_sales_payments_per_page', 20 );
...
//			}
	...
	public function column_date( $payment ) {
		$urls    = array(
			'edit'    => admin_url( 'admin.php?page=eac-sales&edit=' . $payment->id ),
			'delete'  => wp_nonce_url( admin_url( 'admin.php?page=eac-sales&tab=payments&action=delete&id=' . $payment->id ), 'bulk-' . $this->_args['plural'] ),
			'enable'  => wp_nonce_url( admin_url( 'admin.php?page=eac-sales&tab=payments&action=enable&id=' . $payment->id ), 'bulk-' . $this->_args['plural'] ),
			'disable' => wp_nonce_url( admin_url( 'admin.php?page=eac-sales&tab=payments&action=disable&id=' . $payment->id ), 'bulk-' . $this->_args['plural'] ),
		);
		$actions = array(
		...
		return sprintf( '<a href="%1$s">%2$s</a>%3$s', admin_url( 'admin.php?page=eac-sales&tab=payments&edit=' . $payment->id ), wp_kses_post( $payment->name ), $this->row_actions( $actions ) );
	}
