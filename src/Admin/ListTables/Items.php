...
class Items extends ListTable {
	...
	public function prepare_items() {
		...
		$args = array(
			...
			'category_id__in' => eac_filter_input( INPUT_GET, 'category_id', 'absint' ),
		);
	...
	public function process_bulk_action( $doaction ) {
		if ( ! empty( $doaction ) ) {
			$id  = eac_get_request_var( 'item_id', 'get', 0 );
			$ids = eac_get_request_var( 'item_ids', 'get', array() );
			if ( ! empty( $id ) ) {
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			case 'price':
				$value = esc_html( eac_format_price( $item->get_price() ) );
				break;
			case 'unit':
				$unit  = $item->get_unit();
				$units = eac_get_item_units();
