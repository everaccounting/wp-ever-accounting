...
class Terms extends ListTable {
	...
	public function prepare_items() {
		...
		$this->items       = eac_get_terms( $this->_args );
		$this->total_count = eac_get_terms( $this->_args, true );
	...
	public function no_items() {
		esc_html_e( 'No terms found.', 'wp-ever-accounting' );
	}
	...
	public function process_bulk_action( $doaction ) {
		if ( ! empty( $doaction ) && check_admin_referer( 'bulk-' . $this->_args['plural'] ) ) {
			$id  = eac_get_input_var( 'term_id' );
			$ids = eac_get_input_var( 'term_ids', array() );
			if ( ! empty( $id ) ) {
			...
			switch ( $doaction ) {
				case 'delete':
					$notice = __( 'Term(s) deleted successfully.', 'wp-ever-accounting' );
					eac_add_notice( $notice, 'success' );
					break;
				case 'enable':
					$notice = __( 'Term(s) enabled successfully.', 'wp-ever-accounting' );
					eac_add_notice( $notice, 'success' );
					break;
				case 'disable':
					$notice = __( 'Term(s) disabled successfully.', 'wp-ever-accounting' );
					eac_add_notice( $notice, 'success' );
					break;
			}
		}
	 ...
	 *
	 * @param Term $item The current account object.
	 *
	 * @since  1.0.2
	...
	public function column_name( $item ) {
		$args        = array( 'term_id' => $item->get_id() );
		...
		$actions     = array(
			...
			'enable'  => sprintf( '<a href="%s">%s</a>', esc_url( wp_nonce_url( $enable_url, 'bulk-terms' ) ), __( 'Enable', 'wp-ever-accounting' ) ),
			'disable' => sprintf( '<a href="%s">%s</a>', esc_url( wp_nonce_url( $disable_url, 'bulk-terms' ) ), __( 'Disable', 'wp-ever-accounting' ) ),
			'delete'  => sprintf( '<a href="%s" class="del">%s</a>', esc_url( wp_nonce_url( $delete_url, 'bulk-terms' ) ), __( 'Delete', 'wp-ever-accounting' ) ),
		);
