...
class Settings_BK {
	...
	public static function output_fields( $options ) {
		foreach ( $options as $value ) {
			...
			$attrs          = array();
			$value['attrs'] = array_filter( (array) $value['attrs'], 'is_string', ARRAY_FILTER_USE_KEY );
			if ( ! empty( $value['maxlength'] ) ) {
				$value['attrs']['maxlength'] = absint( $value['maxlength'] );
			}
			if ( ! empty( $value['placeholder'] ) ) {
				$value['attrs']['placeholder'] = $value['placeholder'];
			}
			if ( ! empty( $value['autocomplete'] ) ) {
				$value['attrs']['autocomplete'] = $value['autocomplete'] ? 'on' : 'off';
			}
			if ( ! empty( $value['required'] ) ) {
				$value['attrs']['required'] = 'required';
			}
			if ( ! empty( $value['disabled'] ) ) {
				$value['attrs']['disabled'] = 'disabled';
			}
			if ( ! empty( $value['readonly'] ) ) {
				$value['attrs']['readonly'] = 'readonly';
			}
			if ( ! empty( $value['css'] ) ) {
				$value['attrs']['css'] = $value['css'];
			}
			...
			switch ( $value['type'] ) {
				...
				case 'item':
					...
					if ( 'currency' === $value['type'] ) {
						$currencies       = eac_get_currencies( 'active' );
