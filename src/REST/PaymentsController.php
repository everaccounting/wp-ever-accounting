...
class PaymentsController extends TransactionsController {
	...
	/**
	 * Check whether a given request has permission to read payments.
	 *
	...
	/**
	 * Check if a given request has access create payments.
	 *
	 * @param \WP_REST_Request $request Full details about the request.
	 *
	...
	/**
	 * Check if a given request has access to read a payment.
	 *
	 * @param \WP_REST_Request $request Full details about the request.
	 *
	 * @return \WP_Error|boolean
	...
	/**
	 * Check if a given request has access update a payment.
	 *
	 * @param \WP_REST_Request $request Full details about the request.
	 *
	 * @return bool|\WP_Error
	...
	/**
	 * Check if a given request has access delete a payment.
	 *
	 * @param \WP_REST_Request $request Full details about the request.
	 *
	 * @return bool|\WP_Error
	...
	public function get_items( $request ) {
		...
		$results  = \EverAccounting\Payments\query( $args )->get_results( OBJECT, '\EverAccounting\Payments\get' );
		$total    = \EverAccounting\Payments\query( $args )->count();
		$response = array();
	...
	public function create_item( $request ) {
		...
		$item_id = \EverAccounting\Payments\insert( (array) $prepared );
	...
	public function get_item( $request ) {
		...
		$request->set_param( 'context', 'view' );
		$item = \EverAccounting\Payments\get( $item_id );
		if ( is_null( $item ) ) {
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_args ) ) {
			$updated = \EverAccounting\Payments\insert( (array) $prepared_args );
	...
	public function delete_item( $request ) {
		...
		$retval   = \EverAccounting\Payments\delete( $item_id );
		if ( ! $retval ) {
