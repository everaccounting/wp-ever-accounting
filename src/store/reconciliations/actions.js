...
export const fetchReconciliations = (params = {}, reduxer = s => s) => (dispatch, getState) => {
	const state = getState().reconciliations;
	...
	return dispatch({
		...
		payload: apiRequest(accountingApi.reconciliations.list(data)),
...
export const BulkAction = (action, ids) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'RECONCILIATIONS',
		payload: apiRequest(accountingApi.reconciliations.bulk(action, params, removeDefaults(table, 'paid_at'))),
