...
export const fetchCategories = (params = {}, reduxer = s => s) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'CATEGORIES',
		payload: apiRequest(accountingApi.categories.list(data)),
...
export const BulkAction = (action, ids) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'CATEGORIES',
		payload: apiRequest(accountingApi.categories.bulk(action, params, removeDefaults(table, 'paid_at'))),
