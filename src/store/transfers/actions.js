...
export const fetchTransfers = (params = {}, reduxer = s => s) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'TRANSFERS',
		payload: apiRequest(accountingApi.transfers.list(data)),
...
export const BulkAction = (action, ids) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'TRANSFERS',
		payload: apiRequest(accountingApi.transfers.bulk(action, params, removeDefaults(table, 'paid_at'))),
