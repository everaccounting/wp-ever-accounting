...
export const fetchContacts = (params = {}, reduxer = s => s) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'CONTACTS',
		payload: apiRequest(accountingApi.contacts.list(data)),
...
export const BulkAction = (action, ids) => (dispatch, getState) => {
	...
	return dispatch({
		type: 'CONTACTS',
		payload: apiRequest(accountingApi.contacts.bulk(action, params, removeDefaults(table, 'paid_at'))),
