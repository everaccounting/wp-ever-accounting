...
export const setUpdateItem = (id, item) => (dispatch, getState) =>
	updateItem(accountingApi.categories.update, id, item, STATUS_CATEGORIES_ITEM, dispatch, getState().categories);
export const setGetItems = args => (dispatch, getState) =>
	getItems(accountingApi.categories.list, dispatch, STATUS_CATEGORY, args, getState().categories);
...
export const setBulkAction = (action, ids) => (dispatch, getState) =>
	bulkAction(accountingApi.categories.bulk, action, ids, STATUS_CATEGORY, dispatch, getState().categories);
