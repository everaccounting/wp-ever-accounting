...
export const setUpdateItem = (id, item) => (dispatch, getState) =>
	updateItem(
		accountingApi.reconciliations.update,
		id,
		item,
		STATUS_RECONCILIATIONS_ITEM,
		dispatch,
		getState().reconciliations
	);
export const setGetItems = args => (dispatch, getState) =>
	getItems(accountingApi.reconciliations.list, dispatch, STATUS_RECONCILIATION, args, getState().reconciliations);
...
export const setBulkAction = (action, ids) => (dispatch, getState) =>
	bulkAction(
		accountingApi.reconciliations.bulk,
		action,
		ids,
