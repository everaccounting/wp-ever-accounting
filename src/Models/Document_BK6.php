 ...
 */
abstract class Document_BK6 extends Model {
	/**
	 * Table name.
	 *
	...
	*/
	 ...
	 */
	public function get_item( $line_id ) {
		if ( ! empty( $line_id ) && isset( $this->items[ $line_id ] ) ) {
			return $this->items[ $line_id ];
		}
		return false;
	}
	...
	protected function add_item( $data ) {
		...
		if ( is_a( $data, DocumentItem::class ) ) {
			$data = $data->get_data();
		} elseif ( is_a( $data, Item::class ) ) {
			$data = $data->get_data();
			$data = array_merge( $data, array( 'item_id' => $data['id'] ) );
			unset( $data['id'] );
		} elseif ( is_object( $data ) && isset( $data->item_id ) ) {
			$data = get_object_vars( $data );
		} elseif ( is_object( $data ) && isset( $data->id ) && ! isset( $data->item_id ) ) {
			$data = get_object_vars( $data );
			$data = array_merge( $data, array( 'item_id' => $data['id'] ) );
			unset( $data['id'] );
		}

		// Maybe it's a line item from database or a new line item.
		if ( empty( $data['id'] ) && empty( $data['item_id'] ) ) {
			return;
		}

		$line_item = new DocumentItem( $data['id'] );

		// If the line item is not the current document then reset the line item.
		if ( $line_item->exists() && $line_item->get_document_id() !== $this->get_id() ) {
			$line_item->set_defaults();
		}

		if ( ! empty( $data['item_id'] ) ) {
			$item              = eac_get_item( $data['item_id'] );
			$default_data_keys = array_keys( $default_data );
			unset( $default_data['id'] );
			if ( $item ) {
				$default_data = wp_array_slice_assoc( $item->get_data(), $default_data_keys );
			}
		}
		...
		$line_item->set_props( wp_parse_args( $data, $default_data ) );
		...
		// If the line item is already exists then update it.
		if ( ! empty( $line_item->get_id() ) && isset( $this->items[ $line_item->get_id() ] ) ) {
			$this->items[ $line_item->get_id() ] = $line_item;
		} else {
			$this->items['new'] = $line_item;
		}
	}
	...
	public function get_taxes( $id = null, $by_line_id = false ) {
		...
		if ( ! empty( $id ) ) {
			...
			foreach ( $this->taxes as $key => $tax ) {
				if ( $by_line_id && $tax->get_line_id() === $id ) {
					$taxes[ $key ] = $tax;
				} elseif ( $tax->get_item_id() === $id ) {
					$taxes[ $key ] = $tax;
				}
			}
