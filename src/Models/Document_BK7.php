 ...
 */
abstract class Document_BK7 extends Model {
	/**
	 * Table name.
	 *
	...
	protected $core_data = array(
		...
		'total_paid'      => 0.00,
		'total_refunded'  => 0.00,
		'discount_type'   => 'fixed', // applied.
		'discount_amount' => 0.00, // applied.
		'shipping_cost'   => 0.00, // applied.
		'fees_amount'     => 0.00, // applied.
		'notes'           => '',
		'footer'          => '',
		'tax_inclusive'   => 'yes',
	...
	*/
	 ...
	 * @return void
	 */
	public function set_items_v3( $items ) {
		$tax_enabled     = $this->is_calculating_tax() ? 'yes' : 'no';
		$old_items       = array_merge( $this->get_items(), $this->deletable );
		$this->items     = array();
		...
		foreach ( $items as $item_data ) {
			...
			if ( is_object( $item_data ) ) {
				$item_data = $item_data instanceof \stdClass ? get_object_vars( $item_data ) : $item_data->get_data();
			} elseif ( is_numeric( $item_data ) ) {
				$item_data = array( 'product_id' => $item_data );
			}

			// If id and product_id both set then it is update.
			// If id is not set and product_id is set then it is new.

			if ( empty( $item_data['id'] ) && empty( $item_data['product_id'] ) ) {
				return;
			}

			if ( ! empty( $item_data['product_id'] ) ) {
				$product       = eac_get_product( $item_data['product_id'] );
				$product_data  = $product ? $product->get_data() : array();
				$accepted_keys = array( 'name', 'description', 'unit', 'price', 'taxable', 'taxable_shipping', 'taxable_fee', 'tax_ids' );
				$product_data  = wp_array_slice_assoc( $product_data, $accepted_keys );
				$item_data     = wp_parse_args( $item_data, $product_data );
			}

			$item_data = wp_parse_args( $item_data, $default );

			// remove html tags from name and description.
			// set the description limit to 20.
			$item_data['name']        = wp_strip_all_tags( $item_data['name'] );
			$item_data['description'] = wp_strip_all_tags( $item_data['description'] );
			$item_data['description'] = wp_trim_words( $item_data['description'], 20, '' );

			$item = new DocumentItem( $item_data['id'] );
			...
			if ( ! $item->exists() && ! empty( $this->get_items() ) ) {
				foreach ( $this->get_items() as $existing_item ) {
					if ( $existing_item->is_similar( $item ) && ! $item->exists() ) {
						$this->items[] = $existing_item->merge( $item );
		...
		foreach ( $old_items as $old_item ) {
			...
			foreach ( $this->items as $item ) {
				if ( $old_item->exists() && $old_item->get_id() === $item->get_id() ) {
					$found = true;
					break;
				}
			}
	 ...
	 * @return void
	 */
	public function set_items_v2( $items ) {
		$old_items       = array_merge( $this->get_items(), $this->deletable );
		$this->items     = array();
		$this->deletable = array();
		...
		foreach ( $old_items as $old_item ) {
			...
			foreach ( $this->items as $key => $item ) {
				if ( $old_item->get_id() === $item->get_id() ) {
					$found = true;
					break;
				} elseif ( $old_item->is_similar( $item ) ) {
					$found               = true;
					$old_item            = $old_item->merge( $item );
					$this->items[ $key ] = $old_item;

					break;
				}
			}
			if ( ! $found && $old_item->exists() ) {
				$this->deletable[] = $old_item;
			}
		}
	}
	 ...
	 * @return void
	 */
	public function add_item_v2( $data ) {
		$tax_enabled = $this->is_calculating_tax() ? 'yes' : 'no';
		$default     = array(
			'id'               => 0, // line id.
			'product_id'       => 0, // item id not line id be careful.
		...
		// any of existing item is similar to this item then merge it.
		foreach ( $this->get_items() as $key => $old_item ) {
			if ( $old_item->is_similar( $item ) && ! $item->exists() ) {
				$this->items[ $key ] = $old_item->merge( $item );

				return;
			} elseif ( $old_item->get_id() === $item->get_id() ) {
				$this->items[ $key ] = $item;
		...
		foreach ( $items as $item ) {
			...
			foreach ( $line_taxes as $line_tax ) {
				foreach ( $tax_rates as $tax_rate ) {
					if ( $line_tax->get_tax_id() === absint( $tax_rate['tax_id'] ) ) {
	...
	*/
	...
	public function get_data( $context = 'edit' ) {
		...
		foreach ( $items as $item ) {
			...
			$props['items']['taxes'] = [];
			foreach ( $item->get_taxes() as $tax ) {
				$props['items']['taxes'][] = $tax->get_data( $context );
			}
		}
		if ( static::META_TYPE ) {
