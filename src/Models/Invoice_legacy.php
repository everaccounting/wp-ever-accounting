...
class Invoice_legacy extends Document {
	...
	public function get_statuses() {
		return eac_get_invoice_statuses();
	}
	...
	public function save() {
		...
		if ( empty( $this->get_contact_id() ) ) {
			return new \WP_Error( 'missing_required', __( 'Customer is required.', 'wp-ever-accounting' ) );
		}

		if ( empty( $this->get_issue_date() ) ) {
			return new \WP_Error( 'missing_required', __( 'Issue date is required.', 'wp-ever-accounting' ) );
		}

		if ( empty( $this->get_due_date() ) ) {
			return new \WP_Error( 'missing_required', __( 'Due date is required.', 'wp-ever-accounting' ) );
		}
	...
	public function add_payment( $args = array() ) {
		...
		$payment->set_props(
			array(
				...
				'payment_method' => eac_clean( $args['payment_method'] ),
				'description'    => eac_clean( $args['description'] ),
		...
		$payment->save();
		$methods = eac_get_payment_methods();
		$method  = $methods[ $payment->get_payment_method() ];
	...
	public function calculate_totals() {
		...
		foreach ( $this->get_items() as $item ) {
			...
			if ( 'tax_subtotal_rounding' !== eaccounting()->settings->get( 'tax_subtotal_rounding', 'tax_subtotal_rounding' ) ) {
				$item_tax = eac_format_decimal( $item_tax, 2 );
			}
