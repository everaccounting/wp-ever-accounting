 ...
 */
abstract class Document_BK4 extends Model {
	/**
	 * Table name.
	 *
	...
	*/
	 ...
	 * @return float
	 */
	public function get_fee_total( $context = 'edit' ) {
	...
	*/
	...
	public function set_fee_amount( $value ) {
		$this->set_prop( 'fee_amount', eac_sanitize_number( $value, 4 ) );
	}
	...
	*/
	...
	public function save() {
		...
		// Remove deleted taxes.
		if ( ! empty( $this->deleted_taxes ) ) {
			foreach ( $this->deleted_taxes as $tax ) {
				$tax->delete();
			}
		}
	...
	*/
	...
	public function add_item( $data ) {
		$default_data = array(
			...
			'shipping_taxable' => $this->is_calculating_tax(),
			'fee_taxable'      => $this->is_calculating_tax(),
			'tax_ids'          => array(),
			'taxes'            => array(),
		);
		...
		$line_item->set_currency( $this->get_currency() );
		...
		foreach ( $tax_ids as $tax_id ) {
			...
			$line_tax->set_currency( $this->get_currency() );
	...
	*/
	...
	public function calculate_subtotals() {
		...
		foreach ( $items as $item ) {
			...
			$subtotal_taxes = eac_calculate_taxes( $subtotal, $taxes, $this->is_tax_inclusive() );
			if ( $this->is_tax_inclusive() ) {
				$subtotal -= array_sum( wp_list_pluck( $subtotal_taxes, 'amount' ) );
			}
			// Now we will increase subtotal_tax by adding taxes.
			$total_tax_amount = 0;
			foreach ( $taxes as $tax ) {
				foreach ( $subtotal_taxes as $subtotal_tax ) {
					if ( $tax->get_tax_id() === $subtotal_tax['tax_id'] ) {
						// if not taxable or vat exempt then set subtotal_tax to 0.
						if ( ! $item->is_taxable() || $this->is_vat_exempt() ) {
							$tax_amount = 0;
						} else {
							$tax_amount = $subtotal_tax['amount'];
						}
						$tax->set_subtotal_tax( $tax->get_subtotal_tax() + $tax_amount );
			...
			$this->set_subtotal_tax( $this->get_subtotal_tax() + $total_tax_amount );
		}
		$this->set_subtotal( round( $this->get_subtotal(), 2 ) );
		$this->set_subtotal_tax( round( $this->get_subtotal_tax(), 2 ) );
	}
	...
	public function calculate_discounts() {
		...
		foreach ( $items as $item ) {
			...
			$taxes          = $this->get_taxes( $item->get_item_id() );
			$discount_taxes = eac_calculate_taxes( $discount, $taxes, $this->is_tax_inclusive() );
			// If the tax is inclusive then we will subtract the tax amount from the discount.
			if ( $this->is_tax_inclusive() ) {
				$discount -= array_sum( wp_list_pluck( $discount_taxes, 'amount' ) );
			}
			// Now we will increase discount_tax by adding taxes.
			$total_tax_amount = 0;
			foreach ( $taxes as $tax ) {
				foreach ( $discount_taxes as $discount_tax ) {
					if ( $tax->get_tax_id() === $discount_tax['tax_id'] ) {
						// if not taxable or vat exempt then set discount_tax to 0.
						if ( ! $item->is_taxable() || $this->is_vat_exempt() ) {
							$tax_amount = 0;
						} else {
							$tax_amount = $discount_tax['amount'];
						}
						$tax->set_discount_tax( $tax->get_discount_tax() + $tax_amount );
						$total_tax_amount += $tax_amount;
					}
				}
			}
			...
			// Document totals.
			$this->set_discount_total( $this->get_discount_total() + $discount );
			$this->set_discount_tax( $this->get_discount_tax() + $total_tax_amount );
		}
		$this->set_discount_total( round( $this->get_discount_total(), 2 ) );
		$this->set_discount_tax( round( $this->get_discount_tax(), 2 ) );
	}
	...
	public function calculate_shipping() {
		...
		foreach ( $items as $item ) {
			$shipping               = (float) $item->get_shipping_cost();
			$taxes                  = $this->get_taxes( $item->get_item_id() );
			$calculate_shipping_tax = apply_filters( $this->get_hook_prefix() . '_calculate_shipping_tax', true, $item, $this );
			$shipping_taxes         = $calculate_shipping_tax ? eac_calculate_taxes( $shipping, $taxes, false ) : array();
			// Now we will increase shipping_tax by adding taxes.
			$total_tax_amount = 0;
			foreach ( $taxes as $tax ) {
				foreach ( $shipping_taxes as $shipping_tax ) {
					if ( $tax->get_tax_id() === $shipping_tax['tax_id'] ) {
						// if not taxable or vat exempt then set shipping_tax to 0.
						if ( ! $item->is_taxable() || $this->is_vat_exempt() ) {
							$tax_amount = 0;
						} else {
							$tax_amount = $shipping_tax['amount'];
						}
						$tax->set_shipping_tax( $tax->get_shipping_tax() + $tax_amount );
						$total_tax_amount += $tax_amount;
					}
				}
			}

			$item->set_shipping_cost( $shipping );
			// Document totals.
			$this->set_shipping_total( $this->get_shipping_total() + $shipping );
			$this->set_shipping_tax( $this->get_shipping_tax() + $total_tax_amount );
		}
		$this->set_shipping_total( round( $this->get_shipping_total(), 2 ) );
		$this->set_shipping_tax( round( $this->get_shipping_tax(), 2 ) );
	}
	...
	public function calculate_fees() {
		...
		foreach ( $items as $item ) {
			$fee              = (float) $item->get_fee_amount();
			$taxes            = $this->get_taxes( $item->get_item_id() );
			$fee_taxes        = eac_calculate_taxes( $fee, $taxes, false );
			$total_tax_amount = 0;
			foreach ( $taxes as $tax ) {
				foreach ( $fee_taxes as $fee_tax ) {
					if ( $tax->get_tax_id() === $fee_tax['tax_id'] ) {
						// if not taxable or vat exempt then set fee_tax to 0.
						if ( ! $item->is_taxable() || $this->is_vat_exempt() ) {
							$tax_amount = 0;
						} else {
							$tax_amount = $fee_tax['amount'];
						}
						$tax->set_fee_tax( $tax->get_fee_tax() + $tax_amount );
						$total_tax_amount += $tax_amount;
					}
				}
			}

			$item->set_fee_amount( $fee );
			// Document totals.
			$this->set_fees_total( $this->get_fees_total() + $fee );
			$this->set_fees_tax( $this->get_fees_tax() + $total_tax_amount );
		}
		$this->set_fees_total( round( $this->get_fees_total(), 2 ) );
		$this->set_fees_tax( round( $this->get_fees_tax(), 2 ) );
	}
	...
	public function calculate_taxes() {
		...
		foreach ( $items as $item ) {
			...
			$total_tax_amount = 0;
			foreach ( $taxes as $tax ) {
				$tax_amount = $tax->get_subtotal_tax() + $tax->get_shipping_tax() + $tax->get_fee_tax() - $tax->get_discount_tax();
				$tax->set_total_tax( $tax_amount );
				$total_tax_amount += $tax_amount;
			}
			$item->set_total_tax( $total_tax_amount );

			// Document totals.
			$this->set_total_tax( $this->get_total_tax() + $total_tax_amount );
		}
		$this->set_total_tax( round( $this->get_total_tax(), 2 ) );
	}
	...
	public function calculate_total() {
		...
		$this->set_total( round( $this->get_total(), 2 ) );
	}
	...
	public function apply_shipping_cost( $amount = 0, $items = array() ) {
		...
		if ( $per_item_shipping > 0 ) {
			foreach ( $items as $item ) {
				...
				$item->set_shipping_cost( $item->get_shipping_cost() + $shipping );
	...
	public function apply_fee_amount( $amount = 0, $items = array() ) {
		...
		if ( $per_item_fee > 0 ) {
			foreach ( $items as $item ) {
				...
				$item->set_fee_amount( $item->get_fee_amount() + $fee );
			...
			if ( $total_fees > 0 && $total_fees < $amount ) {
				$total_fees += $this->apply_fee_amount( $amount - $total_fees, $items );
			}
		} elseif ( $amount > 0 ) {
			foreach ( $items as $item ) {
				$quantity = $item->get_quantity();
	...
	*/
	...
	public function get_formatted_subtotal() {
		return eac_format_money( $this->get_subtotal(), $this->get_currency() );
	}
	...
	public function get_formatted_discount_total() {
		return eac_format_money( $this->get_discount_total(), $this->get_currency() );
	}
	...
	public function get_formatted_shipping_total() {
		return eac_format_money( $this->get_shipping_total(), $this->get_currency() );
	}
	 ...
	 * @return string
	 */
	public function get_formatted_fee_total() {
		return eac_format_money( $this->get_fees_total(), $this->get_currency() );
	}
	...
	public function get_formatted_tax_total() {
		return eac_format_money( $this->get_total_tax(), $this->get_currency() );
	}
	...
	public function get_formatted_itemized_taxes() {
		...
		$list  = array();
		foreach ( $taxes as $tax ) {
			if ( $tax->get_total_tax() > 0 ) {
				$list[ $tax->get_label() ] = $tax->get_formatted_total_tax();
			}
		}
	...
	public function get_formatted_total() {
		return eac_format_money( $this->get_total(), $this->get_currency() );
	}
	...
	public function get_formatted_totals( $itemized_taxes = false ) {
		$totals = array(
			...
			'fees'     => $this->get_formatted_fee_total(),
