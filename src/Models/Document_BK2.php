 ...
 */
abstract class Document_BK2 extends Model {
	/**
	 * Table name.
	 *
	...
	*/
	...
	public function calculate_taxes() {
		foreach ( $this->get_items() as &$line_item ) {
			...
			$regular_taxes  = array();
			foreach ( $this->get_taxes( $line_item->get_id() ) as &$tax ) {
				if ( $tax->is_compound() ) {
					$compound_taxes[] = &$tax;
				} else {
					$regular_taxes[] = &$tax;
			...
			$taxable_amount = $line_item->get_price() * $line_item->get_quantity() + $line_item->get_shipping() + $line_item->get_fee() - $line_item->get_discount();
			...
			if ( $this->is_tax_inclusive() ) {
				...
				foreach ( $compound_taxes as &$compound_tax ) {
					...
					$compound_tax->set_amount( round( $compound_tax->get_amount() + $tax_amount, 2 ) );
				...
				foreach ( $regular_taxes as &$regular_tax ) {
					...
					$regular_tax->set_amount( round( $regular_tax->get_amount() + $taxable_amount - $net_price, 2 ) );
				}
			} else {
				$pre_compounded = 0;
				foreach ( $regular_taxes as &$regular_tax ) {
					$tax_amount = $taxable_amount * ( $regular_tax->get_rate() / 100 );
					$regular_tax->set_amount( round( $regular_tax->get_amount() + $tax_amount, 2 ) );
			...
			// Let add total tax to line item.
			foreach ( $this->get_taxes( $line_item->get_id() ) as $line_tax ) {
				$line_item->set_total_tax( $line_item->get_total_tax() + $line_tax->get_amount() );
			}
		}
	}
