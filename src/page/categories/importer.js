...
const validCols = ['name', 'type', 'status'];
...
export default class CategoryImporter extends Component {
	...
	import = () => {
		...
		getApi(accountingApi.categories.create(item))
			.then(res => {
				this.setState({
	...
	render() {
		...
		if (status === STATUS_COMPLETE) {
			...
			instructions = sprintf(
				__('Processing %d, Imported %d, skipped %d of total %d items'),
				processed,
				imported,
				skipped,
				data.length
			...
			<div className="ea-importer">
				...
				<DropZoneProvider>
					...
					<div className="inline-notice notice-warning">
						<p>
							{__('Allowed file types: CSV, XLS, XLSX. Please,')} <a href="#">{__('download')}</a>{' '}
							{__('the sample file.')}
						</p>
					</div>
