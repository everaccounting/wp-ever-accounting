...
class EditCurrency extends Component {
	...
	render() {
		...
		const {name, rate, precision, symbol, decimal_mark, thousands_separator, symbol_position = 'before', isSaving} = this.state;
			...
			<Modal title={tittle} onRequestClose={onClose}>
				<form onSubmit={this.onSubmit}>
					...
					<SelectControl label={__('Symbol Position')}
								   options={positions}
								   value={find(positions, {value: symbol_position})}
								   before={<Icon icon='text-width'/>}
								   required
								   onChange={(symbol_position) => {
									   this.setState({symbol_position})
								   }}/>
